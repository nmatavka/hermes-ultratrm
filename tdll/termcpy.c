...
BOOL CopyMarkedTextFromTerminal(const HSESSION hSession, void **ppv,
	...
	StrCharStripHostWideString(pechBuf, (long)StrCharGetEcharByteCount(pechBuf), pechBuf);
	...
	CnvrtECHARtoHost(pszOutput, (ULONG)StrCharGetEcharByteCount(pechBuf) + 1,
						pechBuf,StrCharGetEcharByteCount(pechBuf)+1);//mrw:5/17/95
	*(pszOutput + StrCharGetEcharByteCount(pechBuf)) = ECHAR_CAST('\0');
	free(pechBuf);
	pechBuf = NULL;
	*ppv = pszOutput;
	...
	if (ptBeg.y < 0)
		...
		if (iWant > 0)
				...
				while (iGot-- > 0 && yLn < ptEnd.y && yLn < -1)
					...
					for (pachTmp = achTmp, iTmp = 0 ;
						 *lpachTxt != ECHAR_CAST('\n') &&
                         ...
                         // in legacy double-byte versions by taking into account the
					...
					lpachTxt += 1;
					*pachTmp++ = ECHAR_CAST('\r');

					if (fIncludeLF)
						*pachTmp++ = ECHAR_CAST('\n');

					*pachTmp   = ECHAR_CAST('\0');
			...
			if (iGot >= 0)
				...
				for (pachTmp = achTmp ; xCol-- > 0 && *lpachTxt != ECHAR_CAST('\n'); ++pachTmp)
					*pachTmp = *lpachTxt++;
