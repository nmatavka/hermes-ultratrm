...
static unsigned AsciiHEXToInt(WCHAR *sz);
static unsigned calc_crc(register unsigned crc,  WCHAR *data, int cnt);
 ...
 *
 * ARGUMENTS:
 *  WCHAR *acSerialNo - pointer to a string conatining a serial number.
 *
 * RETURNS:
 ...
 *
 */
int IsValidSerialNumber(WCHAR *acSerialNo)
	{
	WCHAR	 acCRCPart[3];
	WCHAR	 acBuffer[MAX_USER_SERIAL_NUMBER + sizeof(WCHAR)];
	int	 len;
	register unsigned crc1;
	unsigned crc2;

	WCHAR_Fill(acBuffer, L'\0', sizeof(acBuffer)/sizeof(WCHAR));
	StrCharCopy((WCHAR *)acBuffer, acSerialNo);
