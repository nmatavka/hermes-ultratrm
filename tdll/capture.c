...
CCFHexit:
	...
	if (pST != (STCAPTURE *)0)
		...
		if (pST->pszTempCaptureName != (LPWSTR)0)
			{
...
int InitializeCaptureFileHandle(const HSESSION hSession, HCAPTUREFILE hCapt)
	...
	assert(pST);
	WCHAR_Fill(acBuffer, L'\0', sizeof(acBuffer) / sizeof(WCHAR)); // REV 8/27/98
	...
	if ( !GetWorkingDirectory(acBuffer, sizeof(acBuffer) / sizeof(WCHAR)) )
		{
		GetCurrentDirectory(sizeof(acBuffer) / sizeof(WCHAR), acBuffer);
		}
	...
	// StrCharCat(acBuffer, L"CAPTURE.TXT");
	...
	if (pST->pszDefaultCaptureName != (LPWSTR)0)
		free(pST->pszDefaultCaptureName);
	pST->pszDefaultCaptureName = (LPWSTR)0;

	if (pST->pszTempCaptureName != (LPWSTR)0)
		free(pST->pszTempCaptureName);
	pST->pszTempCaptureName = (LPWSTR)0;
...
int LoadCaptureFileHandle(HCAPTUREFILE hCapt)
	...
	if (lSize)
		...
		pOld->pszTempCaptureName = (LPWSTR)0;
		}
...
int cpfGetCaptureFilename(HCAPTUREFILE hCapt,
	...
	if (pST != (STCAPTURE *)0)
		{
		if ((pszStr = pST->pszTempCaptureName) == (LPWSTR)0)
			{
			if ((pszStr = pST->pszDefaultCaptureName) == (LPWSTR)0)
...
HMENU cpfGetCaptureMenu(HCAPTUREFILE hCapt)
	...
	if (pST != (STCAPTURE *)0)
		...
		//
		if (!IsMenu(pST->hMenu))
			pST->hMenu = LoadMenu(glblQueryDllHinst(), L"MenuCapture");
...
void CaptureChar(HCAPTUREFILE hCapt, int nFlags, ECHAR cData)
	...
	CnvrtECHARtoWCHAR(cChar, ARRAYSIZE(cChar), cData);
...
void CaptureLine(HCAPTUREFILE hCapt, int nFlags, ECHAR *achStr, int nLen)
	...
	pszStr = (WCHAR *)malloc(((unsigned int)StrCharGetEcharLen(achStr) +
	...
	CnvrtECHARtoHost(pszStr, ((unsigned long)StrCharGetEcharLen(achStr) + 1)
					* sizeof(ECHAR), achStr,
