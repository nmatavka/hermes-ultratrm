	...
	if (hUpd->bUpdateType == UPD_LINE)
		...
		if (pstLine->iLine != -1)
			...
			for (k = pstLine->xEnd - pstLine->xBeg ; k >= 0 ; --k)
				...
				if (*pachEmuTxt == ECHAR_CAST('\0') || *pachEmuTxt == ECHAR_CAST('\xFF'))
					*pachTermTxt = ECHAR_CAST(' ');
		...
		if (pstScrl->iScrlInc != 0)
			...
			for (; i <= m ; ++i)
				...
				for (j = 0 ; j < iCols ; ++j, ++pachTermTxt, ++pachEmuTxt)
					...
					if (*pachEmuTxt == ECHAR_CAST('\0') ||
							*pachEmuTxt == ECHAR_CAST('\xFF'))
						{
						*pachTermTxt = ECHAR_CAST(' ');
						}
	...
	for (iWant=abs(iScrlInc), iGot=0 ; iWant > 0 ; iWant-=iGot, yBeg+=iGot)
		...
		if (iScrlInc < 0)
			...
			for (k=0 ; (i < 0)	&&	(k < iGot) ; ++i, ++k)
				...
				for (j = 0 ; j < MAX_EMUCOLS && *pachBkTxt != ECHAR_CAST('\n') ; ++j)
					{
					assert(*pachBkTxt);
					*pachTxt++ = *pachBkTxt++;
					}
