...
BOOL RegisterBeveledTextClass(HANDLE hInstance)
	...
	if (GetClassInfo(hInstance, BV_TEXT_CLASS, &wndclass) == FALSE)
		{
...
LONG CALLBACK BeveledTextWndProc(HWND hWnd,
	...
	switch (wMsg)
		...
		case WM_CREATE:
			...
			SetWindowLongPtr(hWnd, 0, (LPARAM)psT);
			...
			switch (GetWindowLongPtr(hWnd, GWL_STYLE) & 0x0000FF00)
				{
				case 0:
				default:
		...
		case WM_DESTROY:
			psT = (LPSTEXT)GetWindowLongPtr(hWnd, 0);
		...
		case WM_GETTEXT:
			...
			if (STEXT_OK(psT))
				...
				if (psT->pszText != NULL)
					...
					if (wPar < wLength)
						wLength = wPar;
					memcpy((LPWSTR)lPar, psT->pszText, wLength);
					}
				}
			return ((LONG)wLength);
			}
		...
		case WM_SETTEXT:
			...
			if (STEXT_OK(psT))
				...
				if (pszStr != NULL)
					...
					if (wLength > 0)
						{
						psT->pszText = (LPWSTR)malloc(wLength + 1);
 ...
 *	Whatever RegisterClass returns.
 */
BOOL UnregisterBeveledTextClass(HANDLE hInstance)
	{
	return UnregisterClass(BV_TEXT_CLASS, hInstance);
	}
