		...
		case BROWSE_BTN:
			...
			if (pS)
				...
				GetDlgItemText(hDlg,
								DIRECTORY_TEXT,
								acBuffer,
								(sizeof(acBuffer) / sizeof(WCHAR)) - 1);
				...
				if (pszRet != NULL)
					{
					StrCharCopyN(acBuffer, pszRet, sizeof(acBuffer) / sizeof(WCHAR));
                    acBuffer[sizeof(acBuffer) / sizeof(WCHAR) - 1] = L'\0';
					SetDlgItemText(hDlg, FNAME_EDIT, pszRet);
					free(pszRet);
					pszRet = NULL;
					pszStr = acBuffer;
					pszRet = pszStr;
					for (; *pszStr != L'\0'; pszStr = StrCharNext(pszStr))
						{
						if (*pszStr == L'\\')
							pszRet = pszStr;
						}
