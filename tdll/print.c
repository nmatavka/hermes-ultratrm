...
VOID PrintKillJob(HSESSION hSession)
	...
	if (hPr->hDC)
		...
		if (iStatus >= 0)
			{
			if (HA5G.fIsWin30)
				iStatus = Escape(hPr->hDC, ENDDOC, 0, (LPWSTR)0, NULL);
 ...
 *	LPCWSTR 	pachStr 	- A pointer to the string to print.
 ...
 *
 */
int printString(const HHPRINT hhPrint, LPCWSTR pachStr, int iLen)
	{
	int 	nCharCount;
	int 	nIdx;
    int iPrintableWidth;
	...
	for (nCharCount = nIdx = 0, pszTemp = pachStr ;
		...
		case L'\r':
			...
			WCHAR_Fill(achBuf, L'\0', sizeof(achBuf)/sizeof(WCHAR));
...
void printQueryPrinterInfo( const HHPRINT hhSessPrint, HHPRINT hhPrint )
	...
	if (hhSessPrint->pstDevNames)
		...
		pTemp = (WCHAR *)hhSessPrint->pstDevNames;
		pTemp += hhSessPrint->pstDevNames->wOutputOffset;
		pTemp += StrCharGetByteCount(pTemp) + 1;

		dwSize = (DWORD)(pTemp - (WCHAR*)hhSessPrint->pstDevNames);
...
int printVerifyPrinter(const HPRINT hPrint)
	...
	//
	fRet = OpenPrinter((LPWSTR)hhPrint->achPrinterName, &hPrinter, NULL);
