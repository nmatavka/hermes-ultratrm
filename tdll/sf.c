...
struct stDllSessionFilePointer
	...
	WCHAR *hFilename;		        /* Memory containing the file name */
...
#define HDR_SIZE	256
#define SF_HEADER_BYTES ((int)(HDR_SIZE * sizeof(WCHAR)))

WCHAR pszHdr[HDR_SIZE] =
	{
	L"UltraTerminal Unicode 2.0 -- UltraTerminal data file\r\n"
	L"Please do not attempt to modify this file directly.\r\n"
	L"\r\n\r\n"
	L"\x1A"
	};
...
int32 sfAddToIndex(const int uNdx,
	...
	if (pD->hData == 0)
		...
		pD->hData = (BYTE *)malloc(ROUND_UP(4096));
			memset(pD->hData, 0, SF_HEADER_BYTES);
			pD->dwDataSize = ROUND_UP(4096);
			pD->dwDataUsed = SF_HEADER_BYTES;
		}
 ...
 *
 * ARGUEMENTS:
 *	WCHAR *pszName	 --  the name of the session file.
 *
 * RETURNS:
 *	0 or an error code < 0;
 */
int sfOpenSessionFile(const SF_HANDLE sF, const WCHAR *pszName)
	{
	int uRv = 0;
	int uNdx = sF - 1;
	...
	if (pszName && (StrCharGetStrLength(pszName) > 0))
		 ...
		 */
		asSessionFiles[uNdx].hFilename = (WCHAR *)malloc(FNAME_LEN * sizeof(WCHAR));
		...
		if (hFile == INVALID_HANDLE_VALUE)
			...
			if (asSessionFiles[uNdx].hData == 0)
				...
				pszStr = (WCHAR *)asSessionFiles[uNdx].hData;
				pbPtr = asSessionFiles[uNdx].hData;
				memset(asSessionFiles[uNdx].hData, 0, uSize);

			/*
			 * Read in the header and check it
			 */
			//fread(pszPtr, 1, HDR_SIZE, f);
				if((unsigned long)SF_HEADER_BYTES > uSize)
					{
					CloseHandle(hFile);
					uRv = SF_ERR_FILE_FORMAT;
					goto OSFexit;
					}
				ReadFile(hFile, pbPtr, SF_HEADER_BYTES, &dw, 0);

				if (memcmp(asSessionFiles[uNdx].hData, pszHdr,
							10 * sizeof(WCHAR)) != 0)
					{
				CloseHandle(hFile);
				uRv = SF_ERR_FILE_FORMAT;
				...
				pbPtr += SF_HEADER_BYTES;
				asSessionFiles[uNdx].dwDataUsed += SF_HEADER_BYTES;
			...
			for (;;)
				...
				if (nOrderOK)
					...
					pIx->uIndex = sID;
					pIx->dwSize = dwSZ;
					pIx->dwOffset = (DWORD)(pbPtr - asSessionFiles[uNdx].hData);
					...
					if( (DWORD)dwSZ > uSize )
						...
						ReadFile(hFile, pbPtr, (DWORD)dwSZ, &dw, 0);
						pbPtr += dwSZ;
					asSessionFiles[uNdx].dwDataUsed += dwSZ;
					}
				else
					{
					WCHAR acBuffer[FNAME_LEN];
...
int sfFlushSessionFile(const SF_HANDLE sF)
	...
	if (asSessionFiles[uNdx].uChanged != 0)
		 ...
		 */
			if (StrCharGetStrLength((WCHAR *)pbPtr) == 0)
				{
				/*
				 * Write out an empty header
				 */
				WriteFile(hFile, pszHdr, SF_HEADER_BYTES, &dw, 0);
				}
			else
				{
				/*
				 ...
				 */
				WriteFile(hFile, pbPtr, SF_HEADER_BYTES, &dw, 0);
				}
		...
		for (x = 0; x < asSessionFiles[uNdx].uItemCount; x += 1)
			...
			if (pI->dwSize != 0)
				...
				WriteFile(hFile, &pI->dwSize, sizeof(DWORD), &dw, 0);
					...
					WriteFile(hFile, pbPtr, (size_t)pI->dwSize,
					    &dw, 0);
 ...
 *	ZERO if everything is OK, otherwise an error code < 0;
 */
int sfGetSessionFileName(const SF_HANDLE sF, const int nSize, WCHAR *pszName)
	{
	int uNdx = sF - 1;
    WCHAR *pszStr;
#if DEADWOOD
    ...
    MemCopy(pszName, pszStr, (size_t)len * sizeof(WCHAR));
    pszName[nSize-1] = L'\0';
#else
    StrCharCopyN(pszName, pszStr, nSize);
#endif
 ...
 */
int sfSetSessionFileName(const SF_HANDLE sF, const WCHAR *pszName)
	{
	unsigned int uNdx = (unsigned int)sF - 1;
	WCHAR *pszStr;
	...
	WCHAR_Fill(pszStr, L'\0', FNAME_LEN);
	StrCharCopyN(pszStr, (LPWSTR)pszName, FNAME_LEN);
