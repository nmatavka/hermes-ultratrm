 ...
 */
long regOpenKey(HKEY hKey, LPCWSTR pszSubKey, REGSAM samAccess, HKEY* phOpenKey)
    {
 ...
 */
 int regQueryValue(HKEY hKey, LPCWSTR pszSubKey, LPCWSTR pszValue,
 ...
 */
int regSetStringValue(HKEY hKey, LPCWSTR pszSubKey, LPCWSTR pszValue,
        LPCWSTR pszData)
    {
    long  lResult = ERROR_SUCCESS;
    DWORD dwSize = 0;
    ...
    if ( lResult == ERROR_SUCCESS )
        ...
        //
        dwSize = StrCharGetByteCount(pszData) + sizeof(WCHAR);
 ...
 * AUTHOR:  C. Baumgartner, 11/27/96
 */
int regSetDwordValue(HKEY hKey, LPCWSTR pszSubKey, LPCWSTR pszValue,
        DWORD dwData)
    {
    long  lResult = ERROR_SUCCESS;
    HKEY  hSubKey = 0;
