...
//STATIC_FUNC int	  propGetIdFromEmuName(LPWSTR pacEmuName);
STATIC_FUNC LRESULT prop_WM_CMD(const HWND hwnd,
								const int nId,
						        const int nNotify,
								const HWND hwndCtrl);
STATIC_FUNC int propValidateBackscrlSize(HWND hDlg);
STATIC_FUNC int propValidateTelnetId(HWND hDlg, WCHAR * pszTelnetId,
                                     int iMaxChars);
...
STATIC_FUNC LRESULT prop_WM_CMD(const HWND hDlg, const int nId,
	...
	BOOL		fResult;
#ifdef INCL_USER_DEFINED_BACKSPACE_AND_TELNET_TERMINAL_ID
    WCHAR       achTempTelnetId[EMU_MAX_TELNETID];
#endif
...
void propLoadEmulationCombo(const HWND hDlg, const HSESSION hSession)
	...
	WCHAR_Fill(acEmuName, L'\0', sizeof(acEmuName) / sizeof(WCHAR));
	emuQueryName(hEmulator, acEmuName, sizeof(acEmuName) / sizeof(WCHAR));
	...
	SendDlgItemMessage(hDlg, IDC_TERMINAL_CB_EMULATION, CB_SELECTSTRING,
		(WPARAM)-1, (LPARAM)(LPWSTR)acEmuName);
	}
#endif
...
void propLoadEmulationCombo(const HWND hDlg, const HSESSION hSession)
	...
	for (i = IDS_EMUNAME_BASE ; i < IDS_EMUNAME_BASE + NBR_EMULATORS; i++)
		...
		if (i == IDS_EMUNAME_VIDEOTEX)
			continue;
		#endif

		LoadString(glblQueryDllHinst(), (unsigned int)i, achText, sizeof(achText) / sizeof(WCHAR));

		SendDlgItemMessage(hDlg, IDC_TERMINAL_CB_EMULATION, CB_ADDSTRING, 0,
				(LPARAM)(LPWSTR)achText);
		}
...
int propGetEmuIdfromEmuCombo(HWND hDlg, HSESSION hSession)
	...
	if ((nRet = SendDlgItemMessage(hDlg, IDC_TERMINAL_CB_EMULATION, CB_GETCURSEL, 0, 0)) == CB_ERR)
		...
		{
		WCHAR_Fill(acEmulator, L'\0', sizeof(acEmulator) / sizeof(WCHAR));

		SendDlgItemMessage(hDlg, IDC_TERMINAL_CB_EMULATION, CB_GETLBTEXT,
			(WPARAM)nRet, (LPARAM)(LPWSTR)acEmulator);
...
STATIC_FUNC void prop_WM_INITDIALOG_Terminal(HWND hDlg)
	...
	LoadString(glblQueryDllHinst(), IDS_XD_INT, ach, sizeof(ach) / sizeof(WCHAR));
	...
	SendDlgItemMessage(hDlg, IDC_TERMINAL_EF_BACKSCRL, WM_SETTEXT, 0, (LPARAM)(LPWSTR)acBuffer);
    ...
    SendDlgItemMessage(hDlg, IDC_TERMINAL_EF_TELNETID, WM_SETTEXT, 0,
        (LPARAM)(LPWSTR)pS->stEmuSettings.acTelnetId);
...
STATIC_FUNC int propValidateTelnetId(HWND hDlg, WCHAR * pszTelnetId,
    ...
    // invalid.
    //
    pszTempCopy = malloc(iMaxChars * sizeof(WCHAR));
    if (pszTempCopy == NULL)
        {
        return -1;
        }
	WCHAR_Fill(pszTempCopy, L'\0', iMaxChars);
