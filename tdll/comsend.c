 ...
 */
int ComSendChar(const HCOM pstCom, const WCHAR chCode)
	{
	assert(ComValidHandle(pstCom));


        while (pstCom->nSBufrSize > 0 &&
               (pstCom->nSendCount >= pstCom->nSBufrSize))
                {
	...
	*pstCom->puchSendPut++ = (BYTE)(chCode & 0x00FF);
	++pstCom->nSendCount;
	return TRUE;
	}
 ...
 */
int ComSendCharNow(const HCOM pstCom, const WCHAR chCode)
	{
	assert(ComValidHandle(pstCom));
