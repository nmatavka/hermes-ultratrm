...
static BOOL InitDll(const HINSTANCE hInstance)
    ...
    #if defined(INCL_PRIVATE_EDITION_BANNER)
    gRTFInstanceHandle = LoadLibraryW(L"RICHED32");
    #endif
...
BOOL InitInstance(const HINSTANCE hInstance,
    ...
    //
    if ( !IsUltraTerminalDefaultTelnetApp() && QueryTelnetCheckFlag() )
        {
#ifndef NT_EDITION
 ...
 *      nSize             Size of buffer pointed to by pachFileName (in wide characters)
 *
 * RETURNS:
 *
 *
 */
int GetFileNameFromCmdLine(WCHAR *pachCmdLine, WCHAR *pachFileName, int nSize)
	{
	int                 nIdx = 0;
	WCHAR *             pszStr;
	WCHAR               acName[FNAME_LEN];
	WCHAR               acPath[FNAME_LEN];
	WCHAR               ach[20];
	WCHAR *             pachFile;
	WCHAR *             pszTelnet = L"telnet:"; //jmh 3/24/97
    ...
    //
    WCHAR_Fill(pachFileName, L'\0', nSize);
    WCHAR_Fill(acName, L'\0', sizeof(acName) / sizeof(WCHAR));
    WCHAR_Fill(acPath, L'\0', sizeof(acPath) / sizeof(WCHAR));
    ...
    if ( pachCmdLine[0] == L'\0' )
        {
        return 0;
        }
    
    for (pszStr = pachCmdLine;
			*pszStr != L'\0' && nIdx < nSize;
		...
		if (*pszStr == L'/')
			...
			if (nIdx < (sizeof(acName) / sizeof(WCHAR) - 1))
				{
				// JFH:6/9/95 acName[nIdx++] = *pszStr;
				if (FALSE)
					{
					MemCopy(&acName[nIdx], pszStr, (size_t)2 * sizeof(WCHAR));
					nIdx += 2;
					}
				else
					{
...
// to make sure we're comparing the same name as in other instances.
	...
	// the code to follow  jkh, 03/22/1997
	    if (fTelnetCmdLnDial)
	        ...
	        if (CompareStringOrdinal(acName, nIdx, pszTelnet, nIdx, TRUE) ==
					CSTR_EQUAL)
			{
    ...
    pszStr = StrCharFindFirst(acName, L':');
	    if (pszStr && pszStr[1] >= L'0' && pszStr[1] <= L'9')
	        {
	        *pszStr = L'\0';
	        }
#endif
	...
	// carry on.
	//
	if (GetFullPathName(acName, sizeof(acPath) / sizeof(WCHAR), acPath, &pachFile) > 0)
		{
		StrCharCopyN(acName, acPath, sizeof(acName) / sizeof(WCHAR));
		}

	// Get ultraterminal extension
	//
    WCHAR_Fill(ach, L'\0', sizeof(ach) / sizeof(WCHAR));
	...
	if ((pszStr = StrCharFindLast(acName, '.')))
        ...
        if (StrCharCmpi(pszStr, ach) != 0 && StrCharCmpi(pszStr, ".TRM") != 0)
            ...
            if(nIdx > nSize)
                {
                ach[nSize - StrCharGetStrLength(ach) - 1] = L'\0';
                }
        ...
        if(nIdx > nSize)
            {
            acName[nSize - StrCharGetStrLength(ach) - 1] = L'\0';
            }
	...
	if (acName[0] != L'\0')
		...
		if (pstFindData)
			...
			memset(pstFindData, 0, sizeof(WIN32_FIND_DATA));
				...
				hSearchHdl = FindFirstFile(acName, pstFindData);
			if (hSearchHdl != INVALID_HANDLE_VALUE)
				{
				// Keep the path for later...
				//
				WCHAR_Fill(acPath, L'\0', sizeof(acPath) / sizeof(WCHAR));
				StrCharCopyN(acPath, acName, sizeof(acPath) / sizeof(WCHAR));
				mscStripName(acPath);
...
BOOL CALLBACK CheckInstCallback(HWND hwnd, LPARAM lPar)
	...
	BOOL  fRet = TRUE;
	WCHAR szClass[256];
	GetClassName(hwnd, szClass, sizeof(szClass) / sizeof(WCHAR));

	if (StrCharCmpi(szClass, SESSION_CLASS) == 0) // mrw, 2/12/95
		{
		ATOM aFile = GlobalAddAtom((WCHAR *)lPar);
 ...
 *
 * DESCRIPTION:
 *      Checks if another instance of UltraTerminal is using this file.
 *
 * ARGUMENTS:
...
static int HTCheckInstance(WCHAR *pachCmdLine)
	{
	WCHAR achPath[FNAME_LEN];

    // Make sure the filename buffer is nulled out.  REV: 11/14/2000.
    //
    WCHAR_Fill(achPath, L'\0', sizeof(achPath) / sizeof(WCHAR));

	// Get the filename
	//
	GetFileNameFromCmdLine(pachCmdLine, achPath, sizeof(achPath) / sizeof(WCHAR));
