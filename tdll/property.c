 ...
 *	Nothing.
 */
void propUpdateTitle(HSESSION hSession, HWND hDlg, LPWSTR pachOldName)
	{
	HWND	hwnd = GetParent(hDlg);
	WCHAR	acTitle[256], acName[256], acNewTitle[256];
	LPWSTR	pszStr, pszStr2;

	GetWindowText(hwnd, acTitle, sizeof(acTitle));
	sessQueryName(hSession, acName, sizeof(acName));
	if (acName[0] != L'\0')
		{
		WCHAR_Fill(acNewTitle, L'\0', sizeof(acNewTitle) / sizeof(WCHAR));
		...
		//
		// if ((pszStr = (LPWSTR)strstr(acTitle, pachOldName)) != NULL)
		if ((pszStr = StrCharStrStr(acTitle, pachOldName)) != NULL)
			{
			for (pszStr2 = pszStr;
				 *pszStr2 || *pachOldName != L'\0';
