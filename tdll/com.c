...
int ComInitHdl(const HCOM pstCom)
	...
	pstCom->stWorkSettings.szDeviceFile[0] = (WCHAR)0;
	pstCom->stWorkSettings.szPortName[0] = (WCHAR)0;
	...
	pstCom->hDriverModule  = (HANDLE)0;
	pstCom->szDeviceName[0]= (WCHAR)0;
	pstCom->chDummy 	   = (WCHAR)0;
	pstCom->afOverride	   = 0;
 ...
 *
 */
int ComSetDeviceFromFile(const HCOM pstCom, const WCHAR * const pszFileName)
	{
	int    iRetVal = COM_OK;
 ...
 *
 */
int ComSetPortName(const HCOM pstCom, const WCHAR * const pszPortName)
	{
	int iRetVal = COM_OK;
 ...
 *
 */
int ComGetPortName(const HCOM pstCom, WCHAR * const pszName, int * const pnLen)
	{
	int iRetVal = COM_OK;
	int nTheirLen = *pnLen;
 ...
 *
 */
int ComRcvBufrRefill(const HCOM pstCom, WCHAR * const tc, const int fRemoveChar)
	{
	int iRetVal;
 ...
 *
 */
int ComDriverSpecial(const HCOM pstCom, const WCHAR * const pszInstructions,
						 WCHAR * const pszResults, const int nBufrSize)
	{
...
void ComReportError(const HCOM pstCom, int iErrStr,
		const WCHAR * const pszOptInfo, const int fFirstOnly)
	{
