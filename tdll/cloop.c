            ...
            for (nRcvCount = 10; nRcvCount--; ) {
                ...
                if (mComRcvChar(hCom, &chData)) {
                    ...
                    if (hhEmu != NULL &&
                        ...
                        int  iHostWideBufferLength = sizeof(HostWideBuffer);
                        DoUTF8 = TRUE;
                        if (TranslateUTF8ToHost(chData,
                                                UTF8Buffer,
                                                iUTF8BufferLength,
                                                UNICODEBuffer,
                                                iUNICODEBufferLength,
                                                HostWideBuffer,
                                                iHostWideBufferLength) == TRUE) {
                            //
                            // Now copy the legacy double-byte buffer back into the chBuffer
                            // character array that is used later.
                            //
                            StrCharCopyN(chBuffer, HostWideBuffer,
                                         sizeof(chBuffer));
                    ...
                    for (nIndx = 0; nIndx < nCount; nIndx++) {
                        if (pstCLoop->fDoHostWide
                            ...
                            echData     = ECHAR_CAST(chBuffer[nIndx]);
                            fValidChar  = TRUE;
#if defined(INCL_VTUTF8)
...
// UNDO:REV 03/01/2001  I believe this is where the problem with slow text file transfer is occurring!
                ...
                if (keyOut == L'\t' &&
                    ...
                    //* while (i-- > 0)
                    //* 	(VOID)mEmuKbdIn(hEmu, (KEY_T)L' ', FALSE);
                }
...
void CLoopCharIn(ST_CLOOP *pstCLoop, ECHAR chIn) {
    ...
    if (pstCLoop->stWorkSettings.fEchoplex) {
        ...
        //mpt:1-27-98 converting echars to wide character was converting
        //            the character to a single byte. Consequently,
        //            echoing of a dbcs character was only echoing
        //            the second byte of the character.
        if ( (pstCLoop->fDoHostWide) && (chIn > 255) ) {
            //send first byte
            ComSendCharNow(pstCLoop->hCom, (WCHAR) (chIn >> 8));
            //send second byte
            ComSendCharNow(pstCLoop->hCom, (WCHAR) (chIn & 0xFF));
        } else {
            ComSendCharNow(pstCLoop->hCom, (WCHAR) chIn);
        }

        if (chIn == ECHAR_CAST('\r')) {
            ComSendCharNow(pstCLoop->hCom, L'\n');
        }
    }
    ...
    if (!pstCLoop->fSuppressDsp) {
        ...
        if (chIn == ECHAR_CAST('\r') && pstCLoop->stWorkSettings.fAddLF)
            emuDataIn(pstCLoop->hEmu, ECHAR_CAST('\n'));
    }
}
 ...
 *
 */
void CLoopCharOut(HCLOOP hCLoop, WCHAR chOut) {
    ...
    for (nIndx = 0; nIndx < nCount; nIndx += 1) {
        ...
        if (hhEmu != NULL && hhEmu->stUserSettings.nEmuId == EMU_VTUTF8) {
            ...
            if (TranslateHostToUTF8(HostWideBuffer,
                                    iHostWideBufferLength,
                                    UNICODEBuffer,
                                    iUNICODEBufferLength,
                                    UTF8Buffer,
...
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 * FUNCTION:
 *	CLoopByteOut / CLoopBytesOut
 *
 * DESCRIPTION:
 *	Send protocol bytes out the transport. Unicode builds keep terminal
 *	cells wide, but emulator escape sequences and wire payloads stay bytes.
 */
void CLoopByteOut(HCLOOP hCLoop, BYTE bOut)
	{
	CLoopBytesOut(hCLoop, &bOut, 1);
	}

void CLoopBytesOut(HCLOOP hCLoop, const void *pvOut, int nLen)
	{
	ST_CLOOP * const pstCLoop = (ST_CLOOP *)hCLoop;
	const BYTE *pbOut = (const BYTE *)pvOut;

	if (pstCLoop == NULL || pbOut == NULL || nLen <= 0)
		return;

	while (nLen--)
		{
		const BYTE bOut = *pbOut++;
		ComSendChar(pstCLoop->hCom, (WCHAR)bOut);

		if (pstCLoop->stWorkSettings.fLocalEcho &&
				!pstCLoop->fSuppressDsp)
			{
			emuDataIn(pstCLoop->hEmu, (ECHAR)bOut);
			emuComDone(pstCLoop->hEmu);
			}
		}
 ...
 *
 */
void CLoopBufrOut(HCLOOP hCLoop, WCHAR *pchOut, int nLen) {
    ST_CLOOP * const pstCLoop = (ST_CLOOP *)hCLoop;
    int fCharValid = FALSE;
        ...
        if (pstCLoop->stWorkSettings.fLocalEcho &&
            ...
            if (pstCLoop->fDoHostWide) {
                if (pstCLoop->cLocalEchoLeadByte != 0) {
                    *pchOut |= (WCHAR)(pstCLoop->cLocalEchoLeadByte << 8);
                    pstCLoop->cLocalEchoLeadByte = 0;
                    fCharValid = TRUE;
                } else {
                    if (FALSE) {
