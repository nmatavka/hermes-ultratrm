 ...
 *
 */
void DoAssertDebug(WCHAR *file, int line)
	{
#if !defined(NDEBUG)
	int retval;
	WCHAR buffer[256];
	...
	wsprintf(buffer,
			L"Assert error in file %s on line %d.\n"
			L"Press YES to continue, NO to call CVW, CANCEL to exit.\n",
			file, line);

	retval = MessageBox((HWND)0, buffer, L"Assert",
 ...
 *
 * ARGUMENTS:
 *	LPWSTR	achFmt	- printf style format string.
 ...
 *
 */
VOID __cdecl DoDbgOutStr(WCHAR *achFmt, ...)
	{
#if !defined(NDEBUG)
	va_list valist;
	WCHAR	achBuf[256];
 ...
 *
 */
void DoShowLastError(const WCHAR *file, const int line)
	{
#if !defined(NDEBUG)
	int retval;
	WCHAR ach[256];
	...
	wsprintf(ach, L"GetLastError=0x%x in file %s, on line %d\n"
				  L"Press YES to continue, NO to call CVW, CANCEL to exit.\n",
				  dwErr, file, line);

	retval = MessageBox((HWND)0, ach, L"GetLastError",
		MB_ICONEXCLAMATION | MB_YESNOCANCEL | MB_SETFOREGROUND);
