...
STATIC_FUNC int LoadTranslateDll(HTRANSLATE pH)
	...
	WIN32_FIND_DATA stF;
	WCHAR *pszNamePart;
	WCHAR szFileName[FNAME_LEN];
	WCHAR szPath[FNAME_LEN];
	...
	if (sH != INVALID_HANDLE_VALUE)
		...
		*pszNamePart = L'\0';
		...
		if (hI->hInstance)
			...
			pstH->pfnCreate = (VOID *(*)(HSESSION))
				GetProcAddress(hI->hInstance, "transCreateHandle");
			/* Do we need to error check these things ? */
			pstH->pfnInit = (int (*)(VOID *))
				GetProcAddress(hI->hInstance, "transInitHandle");
			pstH->pfnLoad = (int (*)(VOID *))
				GetProcAddress(hI->hInstance, "transLoadHandle");
			pstH->pfnSave = (int (*)(VOID *))
				GetProcAddress(hI->hInstance, "transSaveHandle");
			pstH->pfnDestroy = (int (*)(VOID *))
				GetProcAddress(hI->hInstance, "transDestroyHandle");
			pstH->pfnDoDialog = (int (*)(HWND, VOID *))
				GetProcAddress(hI->hInstance, "transDoDialog");
			pstH->pfnIn = (int (*)(VOID *, WCHAR, int *, int, WCHAR *))
				GetProcAddress(hI->hInstance, "transCharIn");
			pstH->pfnOut = (int (*)(VOID *, WCHAR, int *, int, WCHAR *))
				GetProcAddress(hI->hInstance, "transCharOut");
 ...
 *
 */
static int translateInternalCio(VOID *pV, WCHAR cC, int *nR, int nS, WCHAR *pC)
	{
	if (nS > 0)
		{
		*nR = 1;
