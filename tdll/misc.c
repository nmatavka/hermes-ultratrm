 ...
 *  the end of it.
 *
 *  NOTE: I've attempted to make this code legacy double-byte aware.
 *
 * ARGUMENTS:
...
LPWSTR mscModifyToFit(HWND hwnd, LPWSTR pszStr)
	...
	//
	GetTextExtentPoint(hDC, (LPCWSTR)pszStr, StrCharGetStrLength(pszStr), &sz);
	if (sz.cx > nWidth)
		{
		LoadString(glblQueryDllHinst(), IDS_GNRL_ELLIPSIS,
			achEllipsis, sizeof(achEllipsis) / sizeof(WCHAR));
		...
		while ((i > 0) && (sz.cx > nWidth))
			{
			GetTextExtentPoint(hDC, (LPCWSTR)ach, i, &sz);
			i -= 1;
			}
		WCHAR_Fill(ach, L'\0', sizeof(ach) / sizeof(WCHAR));
...
int mscCreatePath(const WCHAR *pszPath)
	...
	while (1)
		{
		if ((pachTok = StrCharFindFirst(pachTok, L'\\')) == 0)
			{
...
int mscAskWizardQuestionAgain(void)
	...
	DWORD	dwSize;
	DWORD	dwType;
	WCHAR	*pszAppKey = "ULTRATERMINAL";

	dwSize = sizeof(DWORD);

	lResult = RegQueryValueEx(HKEY_CLASSES_ROOT, (LPWSTR)pszAppKey, 0,
