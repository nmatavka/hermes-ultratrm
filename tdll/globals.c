 ...
 *
 * RETURNS:
 *	LPWSTR - pointer to the help file name.
 */
LPWSTR glblQueryHelpFileName(void)
	{
	return ((LPWSTR)szHelpFileName);
	}
...
void glblSetHelpFileName(void)
	...
	WCHAR_Fill(szHelpFileName, L'\0', dwSize/sizeof(WCHAR));
	LoadString(glblQueryDllHinst(), IDS_GNRL_HELPFILE, szHelpFileName,
               dwSize / sizeof(WCHAR));

    if ( RegOpenKeyEx(HKEY_LOCAL_MACHINE,
                       L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\UltraTerminal.exe",
                       0,
                       KEY_QUERY_VALUE,
                       &hKey) == ERROR_SUCCESS)
        {
	    ...
	    WCHAR_Fill(szHelpFileName, L'\0', dwSize/sizeof(WCHAR));
        if (RegQueryValueEx(hKey, L"HelpFileName", 0, 0,
