...
struct stSessionData
	...
	WCHAR				achSessCmdLn[FNAME_LEN + 1];	// passed in last parameter of CreateWindow()
  ...
  //HICON				hLittleIcon;		// small copy of previous icon, if available
	...
	//
	WCHAR				achSessName[FNAME_LEN + 1];		// name of session.
	WCHAR				achOldSessName[FNAME_LEN + 1];	// shadow of the session name.
...
int fTestOpenOldTrmFile(const HHSESSION hhSess, WCHAR *achName);
