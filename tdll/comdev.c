...
int ComFindDevices(COM_FIND_DEVICE * const pstFind)
	...
	int 	  fResult;
	HINSTANCE hModule;
	int 	  (*pfDeviceGetInfo)(unsigned *, WCHAR *, int);
		...
		while (iRetVal == COM_OK && !fFound)
			...
			if (!fResult)
				...
				if (hModule)
					...
					pfDeviceGetInfo = (int (*)(unsigned *, WCHAR *, int))
 ...
 */
int ComFindPorts(COM_FIND_PORT * const pstFind, const WCHAR * const pszFileName,
 ...
 */
int ComGetFileNameFromDeviceName(const WCHAR * const pszDeviceName,
		WCHAR * const pszFileName, const int nSize)
	{
