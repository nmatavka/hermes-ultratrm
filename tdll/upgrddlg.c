...
DWORD CALLBACK EditStreamCallback(DWORD_PTR dwCookie, LPBYTE pbBuff,
    ...
    iLen = lstrlen(pbText) + 1;     // get len in chars (legacy double-byte returns bytes)
    iLen *= sizeof(WCHAR);          // adjust for UNICODE
    ...
    case WM_INITDIALOG:
		...
		if (!bUseRTF)
			...
			for (i = IDS_UPGRADE ; i < IDS_UPGRADE + 50 ; ++i)
				{
				if (LoadString(glblQueryDllHinst(), i, ach, sizeof(ach) / sizeof(WCHAR)) == 0)
					break;
...
INT_PTR DoUpgradeDialog(HWND hwndParent)
	...
	WCHAR acHTMFile[MAX_PATH];
    DWORD dwSize = sizeof(acRegistryData);
	BOOL bLaunchBrowser = FALSE;
	WCHAR achHTMRegKey[] = L".htm";
	LPWSTR pszPtr;

    //get .htm file type
		if ( regQueryValue(HKEY_CLASSES_ROOT, achHTMRegKey,
			L"", (LPBYTE)acRegistryData, &dwSize) == 0 )
			bLaunchBrowser = TRUE;


	if ( bLaunchBrowser )
		{
		// Get the path name of UltraTerminal and build path to HTM file
		//
		acExePath[0] = L'\0';
		...
		if (result != 0)
			{
			pszPtr = StrCharFindLast(acExePath, L'\\');
			*pszPtr = L'\0';
			}
		...
		StrCharCat(acHTMFile, L"\\");
		StrCharCat(acHTMFile, L"readme.htm");
		//check if file exists

			if (GetFileAttributesW(acHTMFile) != INVALID_FILE_ATTRIBUTES)
				{
				...
				ShellExecuteW(NULL, L"open", acHTMFile, NULL, NULL, SW_SHOW);
				return 0;
				}
