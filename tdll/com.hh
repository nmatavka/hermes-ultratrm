...
typedef struct
	{
	WCHAR szDeviceFile[MAX_PATH];			 // Name of driver file
	WCHAR szPortName[COM_MAX_PORT_NAME];	 // Name of individual port
	} ST_COM_SETTINGS;
...
typedef struct s_com
	...
	WCHAR		szDeviceName[COM_MAX_DEVICE_NAME]; // Name of device type
	WCHAR		chDummy;			 // Dummy address for buffer pointers
	...
	// Control	fields for sending:
		BYTE	  * puchSendBufr1;		 // allocated buffers for holding
		BYTE	  * puchSendBufr2;		 //   protocol bytes waiting to be sent
		BYTE	  * puchSendBufr;		 // pointer to bufr being filled
		BYTE	  * puchSendPut;		 // Insertion point in current buffer
		BYTE		auchDummyBufr[5];	 // Place to rest idle pointers
	...
	int 	 (WINAPI *pfDeviceSpecial)(void *, const WCHAR *, WCHAR *, int);
	...
	int 	 (WINAPI *pfPortPreconnect)(void *pvDevData,
						WCHAR *pszPortName, HWND hwndParent);
