...
BOOL InitializeSessionHandle(const HSESSION hSession, const HWND hwnd,
	...
	WCHAR 	ach[256], achTitle[100], achFormat[100];
	...
	if (pcs)
		...
		if (pcs->lpCreateParams)
            ...
            //
			StrCharCopyN(hhSess->achSessCmdLn, (WCHAR*)pcs->lpCreateParams,
                sizeof(hhSess->achSessCmdLn)/sizeof(WCHAR) - 1);

            // Make sure the array is NULL terminated. REV: 11/10/2000
            //
            hhSess->achSessCmdLn[sizeof(hhSess->achSessCmdLn)/sizeof(WCHAR) - 1] = L'\0';
			}
		}
...
BOOL ReinitializeSessionHandle(const HSESSION hSession, const int fUpdateTitle)
	...
	WCHAR_Fill(hhSess->achSessName,
				L'\0',
				sizeof(hhSess->achSessName) / sizeof(WCHAR));
	WCHAR_Fill(hhSess->achOldSessName,
				L'\0',
...
void DestroySessionHandle(const HSESSION hSession)
    ...
    if (hhSess->hTimerMux)
        {
        TimerMuxDestroy(&hhSess->hTimerMux);
...
void sessQueryCmdLn(const HSESSION hSession, LPWSTR pach, const int len)
	{
	int i;
	const HHSESSION hhSess = VerifySessionHandle(hSession);
	WCHAR *pachCmdLn = hhSess->achSessCmdLn;

	for (i = 0 ; i < len ; ++i)
		{
		if (*pachCmdLn == (WCHAR)0)
			break;
...
void sessQueryOldName(const HSESSION hSession, const LPWSTR pach, unsigned uSize)
	{
	const HHSESSION hhSess = VerifySessionHandle(hSession);
...
void sessSetName(const HSESSION hSession, const LPWSTR pach)
	{
	const HHSESSION hhSess = VerifySessionHandle(hSession);
...
void sessQueryName(const HSESSION hSession, const LPWSTR pach, unsigned uSize)
	{
	const HHSESSION hhSess = VerifySessionHandle(hSession);
 ...
 *
 */
BOOL sessIsSessNameDefault(LPWSTR pacName)
	{
	WCHAR ach[FNAME_LEN];
