...
void printsetSetup(const HPRINT hPrint, const HWND hwnd)
	...
	if (hhPrint->pstDevNames)
		...
		// DEVNAMES.
		//
		pTemp = (WCHAR *)hhPrint->pstDevNames;
		pTemp += hhPrint->pstDevNames->wOutputOffset;
		pTemp += StrCharGetByteCount(pTemp) + sizeof(WCHAR);

		dwSize = (DWORD)(pTemp - (WCHAR*)hhPrint->pstDevNames);
	...
	// into the dialog.
	//
	WCHAR_Fill(hhPrint->achPrintToFileName,
				L'\0',
				sizeof(hhPrint->achPrintToFileName) / sizeof(WCHAR));
	...
	pszPrinterName = (WCHAR *)pstDevNames;
	...
	//
	pTemp = (WCHAR *)pstDevNames;
	pTemp += pstDevNames->wOutputOffset;
	pTemp += StrCharGetByteCount(pTemp) + sizeof(WCHAR);

	dwSize = (DWORD)(pTemp - (WCHAR*)pstDevNames);
...
void printsetPrint(const HPRINT hPrint)
	...
	// or to a printer.
	//
	if (hhPrint->achPrintToFileName[0] == L'\0')
		{
		hhPrint->di.lpszOutput = (LPWSTR)NULL;
		}
	else
		{
		hhPrint->di.lpszOutput = (LPWSTR)hhPrint->achPrintToFileName;
		}
...
LRESULT CALLBACK printsetDlgProc(HWND hwnd, UINT uMsg, WPARAM wPar, LPARAM lPar)
	...
	switch (uMsg)
		...
		case WM_INITDIALOG:
			...
			acPtrs[1] = (WCHAR *)hhPrint->pstDevNames +
						hhPrint->pstDevNames->wOutputOffset;
...
static int printsetPrintToFile(const HPRINT hPrint)
	...
	WCHAR_Fill(acDir,		L'\0', sizeof(acDir) / sizeof(WCHAR));
	...
	if (sfGetSessionFileName(sessQuerySysFileHdl(hhPrint->hSession),
		...
		if ( !GetWorkingDirectory( acDir, sizeof(acDir) / sizeof(WCHAR)) )
			{
