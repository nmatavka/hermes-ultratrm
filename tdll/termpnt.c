...
void termPaint(const HHTERM hhTerm, const HWND hwnd)
{
    ...
    if (i < iPaintEnd && i < 0) {
        ...
        for ( ; i < iPaintEnd && i < 0 ; i+=1, n+=iScrlInc) {
            ...
            for (x = 0; x <= j; ) {
                if (isHostWideChar(*(fplpstrTxt[iOffset]+x))) {
            ...
            StrCharStripHostWideString(aechTmpBuf, sizeof(aechTmpBuf),
 ...
 *	int		fForceRight - used for legacy double-byte in the backscroll
    ...
    while (i < nCount) {
        ...
        if (iRow < 0) {
            ...
            if (nCurPos > 0) { // Only copy the buffer if nCurPos > 0. REV: 04/11/2001.
                ...
                nXPos = CnvrtECHARtoHost(achBuf, sizeof(achBuf), aechTmp, (unsigned)nCurPos * sizeof(ECHAR));
            
               //nXPos = StrCharGetByteCount(achBuf); // mrw;5/17/95
            }
            // 18-May-1995	DLW
            // Added for the case where a legacy double-byte string is started with half of
        ...
        nStrLen = CnvrtECHARtoHost(achBuf, sizeof(achBuf), aechTmp,
        ...
        if (!DoUTF8) {
#endif        
#if 0 // mrw:10/10/95 - ifdef'ed this code
        if (!hhTerm->iEvenFont && !(fForceRight && fFirstPass)) {
            // If this is a legacy double-byte character we are painting, then we want to paint 2 cells
            // instead of 1 (makes sense, legacy double-byte characters are twice as wide as SBCS).
            if (FALSE) {
         ...
         ExtTextOut(hdc, nXStart, nYStart, ETO_CLIPPED | ETO_OPAQUE,
            ...
            rc.right += hhTerm->xChar;
            rc.right = min(rc.right, x + (hhTerm->iCols * hhTerm->xChar));
            ExtTextOutW(hdc, nXStart, nYStart, ETO_CLIPPED | ETO_OPAQUE,
                       &rc, (PWSTR)achBuf, (unsigned)nStrLen, 0);
        }
#endif
