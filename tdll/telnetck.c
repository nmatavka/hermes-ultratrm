...
#else
static const WCHAR g_achUltraTerminalRegKey[] =
    L"SOFTWARE\\Microsoft\\UltraTerminal";
#endif

// Registry value for telnet checking:
//
static const WCHAR g_achTelnetCheck[] = L"Telnet Check";

// Registry keys for the web browsers:
//
static const WCHAR g_achIERegKey[] =
    L"SOFTWARE\\Classes\\telnet\\shell\\open\\command";

static const WCHAR g_achNetscapeRegKey[] =
    L"SOFTWARE\\Netscape\\Netscape Navigator\\Viewers";

// Registry values for the browser telnet apps:
//
static const WCHAR g_achIERegValue[] = L"";

static const WCHAR g_achNetscapeRegValue[] = L"telnet";
 ...
 *
 * DESCRIPTION:
 *	Determines if UltraTerminal is the default telnet app for Internet
...
BOOL IsUltraTerminalDefaultTelnetApp(void)
    {
    WCHAR acExePath[MAX_PATH];
    WCHAR acRegistryData[MAX_PATH * 2];
    long  lRet = 0;
 ...
 */
 BOOL QueryTelnetCheckFlag(void)
    ...
    if ( regQueryValue(HKEY_CURRENT_USER, g_achUltraTerminalRegKey,
 ...
 *
 * PARAMETERS:
 *	fCheck - Check if UltraTerminal is the default telnet app?
 *
 * RETURNS:
...
int SetTelnetCheckFlag(BOOL fCheck)
    ...
    if ( regSetDwordValue(HKEY_CURRENT_USER, g_achUltraTerminalRegKey,
