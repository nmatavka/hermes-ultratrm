...
BOOL termValidatePosition(const HHTERM	hhTerm,
	...
	if (pLocation->y <= 0)
		...
		pszStr = (LPWSTR)hhTerm->fplpstrBkTxt[nOffset];
		}
	else if (pLocation->y > 0)
		{
		nOffset = pLocation->y - 1;

		pszStr = (LPWSTR)hhTerm->fplpstrTxt[nOffset];
		}

	assert(pszStr);

	if (pszStr == (LPWSTR)NULL)
		return FALSE;

	/*
	 * We need to loop thru the string and check if the last character before
	 * the specified character is a legacy double-byte lead byte.  If it is, we return FALSE
