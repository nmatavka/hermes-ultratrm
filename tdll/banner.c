...
BOOL bannerRegisterClass(HANDLE hInstance)
	...
	if (GetClassInfo(hInstance, BANNER_DISPLAY_CLASS, &wnd) == FALSE)
		{
		...
		wnd.hIcon               = extLoadIcon(MAKEINTRESOURCE(IDI_ULTRATERMINAL));
 ...
 *
 */
HWND bannerCreateBanner(HANDLE hInstance, LPWSTR pszTitle)
	{
 ...
 */
int fontSetWindowFont(HWND hwnd, LPCWSTR pszFontName, int iPointSize)
    {
...
STATIC_FUNC void banner_WM_CREATE(HWND hwnd, LPCREATESTRUCT lpstCreate)
   ...
   if (lpstCreate->hwndParent)
    ...
    LoadString(glblQueryDllHinst(), IDS_UPGRADE_FONT, ach, sizeof(ach)/sizeof(WCHAR));
	...
	LoadString(glblQueryDllHinst(), IDS_UPGRADE_INFO,
		achUpgradeInfo, sizeof(achUpgradeInfo)/sizeof(WCHAR));
