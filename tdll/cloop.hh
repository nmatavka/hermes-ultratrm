...
#define SF_CLOOP_HOSTWIDTH		    0x20F
...
struct s_cloop_out
	...
	WCHAR		 *puchData; 	   // address of data block
	WCHAR		 *puchLimit;	   // first addr. past data block
	WCHAR		 *puchHead; 	   // where data is added to block
	WCHAR		 *puchTail; 	   // where data is removed from block
	unsigned long ulBytes;		   // size of file being sent
	WCHAR		  chLastChar;	   // last byte out
	};
...
struct s_cloop_settings
	...
	WCHAR	  chWaitChar;				// Char to wait for after sending line
	int 	  fExpandTabsOut;
...
struct s_cloop
	...
	WCHAR		 *panFltrIn;			// Translation table - incoming data
	WCHAR		 *panFltrOut;			// Translation table - outgoing data
	WCHAR		 *panFltrInHold;		// Holding place for the pointer
	WCHAR		 *panFltrOutHold;		// Holding place for the pointer
	...
	HANDLE		  ahEvents[3];			// 0=loop control,
										// 1=receive control
										// 2=send control
	// added for legacy multibyte input support
	int			 fDoHostWide;				// Enables legacy multibyte processing
	WCHAR		 cLeadByte;				// If this is non-zero, then the last
										// character was a lead byte and we are
										// building a multi-byte character
	WCHAR		 cLocalEchoLeadByte;	// Ditto
	};
