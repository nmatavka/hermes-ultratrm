...
extern int	ComSetDeviceFromName(const HCOM pstCom,
				const WCHAR * const pszDeviceName);
extern int	ComSetDeviceFromFile(const HCOM pstCom,
				const WCHAR * const pszFileName);
...
extern int	ComRcvBufrRefill(const HCOM pstCom, WCHAR * const tc, const int fRemoveChar);
...
extern int	ComDriverSpecial(const HCOM pstCom, const WCHAR * const pszInstructions,
				WCHAR * const pszResults, const int nBufrSize);

extern int	ComSendChar(const HCOM pstCom, const WCHAR chCode);
extern int	ComSendCharNow(const HCOM pstCom, const WCHAR chCode);
...
extern int ComLoadSshDriver(HCOM pstCom);
...
#define mComRcvBufrPutback(h,c)  ((VOID)(*--((PCOM)h)->puchRBData = (WCHAR)(c)))
