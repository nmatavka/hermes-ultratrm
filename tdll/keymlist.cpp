...
int Emu_Key_Macro_List :: removeMacro( const KEYDEF & aKey )
    ...
    for ( int i = lIndex; i < mMacroCount - 1; i++ )
        {
...
int Emu_Key_Macro_List :: save( const HSESSION hSession )
    ...
    const string lBuffer = lStream.str();

    if ( sfPutSessionItem( hSF, SFID_KEY_MACRO_LIST,
            (unsigned long)lBuffer.size(), (void *)lBuffer.c_str() ) != 0 )
        {
        lReturnCode = -1;
        }
