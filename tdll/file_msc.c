...
INT InitializeFilesDirsHdl(const HSESSION hSession, HFILES hFile)
	...
	LPWSTR pszRname;
	int nSize;
	WCHAR acDir[FNAME_LEN];

	pFD = (FD_DATA *)hFile;
	assert(pFD);

	pszSname = pszRname = (LPWSTR)0;
	...
	if (pFD)
		...
		if (pFD->pszTransferSendDirectory)
            {
			free(pFD->pszTransferSendDirectory);
		    pFD->pszTransferSendDirectory = (LPWSTR)0;
            }
		...
		if (pFD->pszTransferRecvDirectory)
            {
			free(pFD->pszTransferRecvDirectory);
		    pFD->pszTransferRecvDirectory = (LPWSTR)0;
            }
 ...
 *
 */
LPCWSTR filesQuerySendDirectory(HFILES hFile)
	{
	FD_DATA *pFD;
	...
	if (pFD->pszTransferSendDirectory == (LPWSTR)0)
		return (LPCWSTR)pFD->pszInternalSendDirectory;

	if (StrCharGetStrLength(pFD->pszTransferSendDirectory) == 0)
		return (LPCWSTR)pFD->pszInternalSendDirectory;

	return (LPCWSTR)pFD->pszTransferSendDirectory;
	}
 ...
 *
 */
LPCWSTR filesQueryRecvDirectory(HFILES hFile)
	{
	FD_DATA *pFD;
	...
	if (pFD->pszTransferRecvDirectory == (LPWSTR)0)
		return (LPCWSTR)pFD->pszInternalRecvDirectory;

	if (StrCharGetStrLength(pFD->pszTransferRecvDirectory) == 0)
		return (LPCWSTR)pFD->pszInternalRecvDirectory;

	return (LPCWSTR)pFD->pszTransferRecvDirectory;
	}
 ...
 *
 */
VOID filesSetSendDirectory(HFILES hFile, LPCWSTR pszDir)
	{
	LPWSTR pszTmp;
	FD_DATA *pFD;
 ...
 *
 */
VOID filesSetRecvDirectory(HFILES hFile, LPCWSTR pszDir)
	{
	LPWSTR pszTmp;
	FD_DATA *pFD;
...
int fileBuildFileList(void **pData,
	...
	LPWSTR pszStr;
	LPWSTR *pszArray;
	WCHAR pszLocalDirectory[FNAME_LEN];
	...
	pLocalData = malloc(sizeof(LPWSTR) * FM_CHUNK_SIZE);
	if (pLocalData == NULL)
		nRet = FM_ERR_NO_MEM;
...
STATIC_FUNC int fmBFLinternal(void **pData,
	...
	pszBuildName = (LPWSTR)malloc(FNAME_LEN * sizeof(WCHAR));
	...
	if (sH != INVALID_HANDLE_VALUE)
		...
		do {
			...
			if ((*pCnt > 0) && ((*pCnt % FM_CHUNK_SIZE) == 0))
				...
				*pData = realloc(*pData, nSize * sizeof(LPWSTR) );
				if (*pData == NULL)
					{
					nRet = FM_ERR_NO_MEM;
					goto fmBFLexit;
	...
	if (nSubdir)
		{
		StrCharCopy(pszBuildName, pszDirectory);
		StrCharCat(pszBuildName, L"*.*");	/* This may need to change */
		sH = FindFirstFile(pszBuildName, &stF);
		if (sH != INVALID_HANDLE_VALUE)
			{
			/* Handle is OK, we have something to work on */
			do {
				...
				if (stF.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
					{
					if (StrCharCmp(stF.cFileName, L".") == 0)
						continue;
					if (StrCharCmp(stF.cFileName, L"..") == 0)
						continue;
					StrCharCopy(pszBuildName, pszDirectory);
					StrCharCat(pszBuildName, stF.cFileName);
					StrCharCat(pszBuildName, L"\\");
					fmBFLinternal(pData,
								pCnt,
								pszName,
								nSubdir,
...
static LPWSTR pszBS = L"\\";

int fileFinalizeName(LPWSTR pszOldname,
					LPWSTR pszOlddir,
					LPWSTR pszNewname,
					const size_t cb)
	{
	WCHAR *pachFile;
	WCHAR achCurDir[MAX_PATH];
	...
	LPWSTR pszStr;
	LPWSTR pszFoo;
	WCHAR acWorking[FNAME_LEN];
	WCHAR acDevice[32];
	WCHAR acDirectory[FNAME_LEN];
	WCHAR acName[FNAME_LEN];

	WCHAR_Fill(acWorking, L'\0', sizeof(acWorking) / sizeof(WCHAR));
	WCHAR_Fill(acDevice, L'\0', sizeof(acDevice) / sizeof(WCHAR));
	WCHAR_Fill(acDirectory, L'\0', sizeof(acDirectory) / sizeof(WCHAR));
	WCHAR_Fill(acName, L'\0', sizeof(acName) / sizeof(WCHAR));
	...
	if (*pszPtr == L':')
		{
		/* Copy up thru the ':' */
        if (pszPtr >= (LPWSTR)acWorking)
		    MemCopy(acDevice, acWorking, pszPtr - (LPWSTR)acWorking + 1);
	...
	pszStr = StrCharFindLast(acWorking, L'\\');

	if (acDevice[0] == L'\0')
		if ((acDirectory[0] == L'\\') || (*pszPtr == L'\\'))
			{
			/* TODO: figure out how to replace getdrive */
			// acDevice[0] = (UCHAR)(L'A' + _getdrive() - 1);
			acDevice[0] = L'C';
			acDevice[1] = L':';
			acDevice[2] = L'\0';
			}
	...
	GetCurrentDirectory(sizeof(acWorking) / sizeof(WCHAR), acWorking);
	pszStr = acWorking;
	...
	*pszNewname = L'\0';
	...
	//
	if (acDirectory[1] != L'\\' && acDirectory[1] != L'/')
		{
		StrCharCat(pszNewname, acDevice);
		}

	if ((acDirectory[0] != L'\\') && (acDevice[0] == L'\0'))
		{
 ...
 *
 */
HBITMAP fileReadBitmapFromFile(HDC hDC, LPWSTR pszName, int fCmp)
	{
 ...
 */
int GetFileSizeFromName(WCHAR *pszName, unsigned long * const pulFileSize)
	{
 ...
 *
 */
int GetModuleDirectoryName(HINSTANCE hInst, LPWSTR pszName, int dLen)
	{
	int nRet;
	LPWSTR pszPtr;
	LPWSTR pszPrv;
