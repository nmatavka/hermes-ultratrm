...
MKMF_SRCS   = \
	  ...
	  \wacker\tdll\ut_text.c          \wacker\tdll\update.c       \
...
RCSFILES = \wacker\tdll\makefile.t              \wacker\tdll\tdll.def           \
           ...
           \wacker\term\orderfrm.htm            \wacker\term\UltraTerminal.ico  \
           \
           \wacker\help\hyper_pr.rtf            \wacker\help\UltraTerminal.rtf       \
           \wacker\help\UltraTerminal.cnt            \wacker\help\UltraTerminal.hpj       \
           \wacker\help\cshelp.bmp              \
           \wacker\help\UltraTerminal.hlp            \wacker\term\htpe3bnr.bmp		\
           \wacker\help\UltraTerminal.chm            \
           \
...
TARGETS : \wacker\tdll\ver_dll.i UltraTerminal.dll UltraTerminal.exp UltraTerminal.lib

#-------------------#

CFLAGS += /Fd$(BD)\ultraterminal

%if defined(USE_BROWSER) && $(VERSION) == WIN_DEBUG
CFLAGS += /Fr$(BD)/
TARGETS : UltraTerminal.bsc
%endif

%if defined(MAP_AND_SYMBOLS)
TARGETS : UltraTerminal.sym
%endif
...
LFLAGS += /DLL /entry:TDllEntry $(**,M\.res) /PDB:$(BD)\ultraterminal \
...
UltraTerminal.dll + UltraTerminal.exp + UltraTerminal.lib .MISER : $(OBJS) tdll.def term.res
    @echo Linking $(@,F) ...
    @link $(LFLAGS) $(OBJS:X) /DEF:tdll.def -out:$(@,M\.dll)
    %chdir $(BD)
	@bind UltraTerminal.dll

UltraTerminal.bsc : $(OBJS,.obj=.sbr)
    @echo Building browser file $(@,F) ...
    @bscmake /nologo /o$@ $(OBJS,X,.obj=.sbr)

UltraTerminal.sym : UltraTerminal.map
	mapsym -o $@ $**
