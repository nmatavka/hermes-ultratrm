...
HCLOOP CLoopCreateHandle(const HSESSION hSession)
	...
	if (!pstCLoop)
		...
		pstCLoop->fDoHostWide			= FALSE;
		pstCLoop->cLeadByte			= 0;
		pstCLoop->cLocalEchoLeadByte= 0;
#if 0
		// Added for debugging
		pstCLoop->fDoHostWide			= TRUE;
#endif
...
void CLoopDeactivate(const HCLOOP hCLoop)
	...
	if (pstCLoop)
		...
		// be able to exit
                ...
                if (WAIT_TIMEOUT == WaitForSingleObject (pstCLoop->hEngineThread,0))
			{
			...
			if (WAIT_TIMEOUT == 
                            WaitForSingleObject(pstCLoop->hEngineThread, 10000))
			    {

                            // hEngineThread is not signaled, kill it.

                            if (FALSE == TerminateThread (pstCLoop->hEngineThread, 0))
                                {
                                // If for some reason the thread won't die, we'll 
                                // just return from here and not worry about leaking
                                // because we have leaked already.
                                return;
                                }
                            }

                        // Now we can close the handle and do the rest
                        //  of the cleanups.
...
void CLoopControl(
    ...
    if (bittest(uReason, CLOOP_HOSTWIDTH))
		{
		...
		bitclear(uReason, CLOOP_HOSTWIDTH);
...
void * CLoopRegisterRmtInputChain(const HCLOOP hCLoop,
	...
	if ((pstNew = (ST_FCHAIN *)malloc(sizeof(*pstNew))) != NULL)
		...
		DBGOUT_NORMAL("CLoopRegisterRmtInputChain(%p, %p) -> %p\r\n",
					(void *)(UINT_PTR)pstNew->pfFunc, pstNew->pvUserData,
					pstNew, 0, 0);
...
void CLoopUnregisterRmtInputChain(void *pvHdl)
	...
	DBGOUT_NORMAL("CLoopUnregisterRmtInputChain(%p)\r\n",
				pvHdl, 0, 0, 0, 0);
...
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 * FUNCTION: QueryCLoopHostWidthState
 *
 * DESCRIPTION: Determines the current legacy double-byte mode.
 *	
 * ARGUMENTS:
 *	hCLoop -- Handle returned from call to CLoopCreateHandle
 *
 * RETURNS:
 *    TRUE  -  if we are in legacy double-byte mode.
 *    FALSE -  if we're not
 */
int QueryCLoopHostWidthState(HCLOOP hCLoop)
	{
	ST_CLOOP *pstCLoop = (ST_CLOOP *)hCLoop;

	assert(pstCLoop);
	return(pstCLoop->fDoHostWide);
	}


/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 * FUNCTION: SetCLoopHostWidthState
 *
 * DESCRIPTION:  Turns on/off legacy double-byte mode.
 *	
 * ARGUMENTS:
 *	hCLoop -- Handle returned from call to CLoopCreateHandle
 *           fState -- TRUE,  if we're turning legacy multibyte on,
 *                         FALSE, if we're turning legacy multibyte off.
 *
 * RETURNS:
 *    the value of dDolegacy multibyte from the CLoop's internal struct
 */
int SetCLoopHostWidthState(HCLOOP hCLoop, int fState)
   {
	ST_CLOOP *pstCLoop = (ST_CLOOP *)hCLoop;

	assert(pstCLoop);
	pstCLoop->fDoHostWide = fState;
