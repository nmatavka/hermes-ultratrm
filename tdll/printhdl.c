...
int printInitializeHdl(const HPRINT hPrint)
	...
	if (GetProfileString("Windows", "Device", ",,,", achBuf,
		...
		{
		StrCharCopyN(hhPrint->achPrinterName, pszString, sizeof(hhPrint->achPrinterName) / sizeof(WCHAR));
		}
	else
		{
    ...
    if (hhPrint->lf.lfHeight == 0)
		...
		if ( LoadString(glblQueryDllHinst(), IDS_PRINT_DEF_FONT,
			faceName, sizeof (hhPrint->lf.lfFaceName) / sizeof(WCHAR)) )
			{
			StrCharCopyN( hhPrint->lf.lfFaceName, faceName, sizeof (faceName) / sizeof(WCHAR) );
			hhPrint->lf.lfFaceName[sizeof(hhPrint->lf.lfFaceName)/sizeof(WCHAR)-1] = L'\0';
			}
...
void printSaveHdl(const HPRINT hPrint)
	...
	if (hhPrint->pstDevNames)
		...
		//
		sz = (WCHAR *)hhPrint->pstDevNames +
			hhPrint->pstDevNames->wOutputOffset;

		sz += StrCharGetByteCount((LPCSTR)sz) + sizeof(WCHAR);
		ulSize = (unsigned long)(sz - (WCHAR *)hhPrint->pstDevNames);
