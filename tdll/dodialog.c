 ...
 *
 */
INT_PTR DoDialog(HINSTANCE hInstance, LPCWSTR lpTemplateName,
	...
	sRetVal = DialogBoxParam(hInstance, lpTemplateName, hwndParent,
		...
		if (sRetVal == -1)
			{
			WCHAR str[128], awch[50];

			OemToChar("Couldn't load %s. (%s, %d)", awch);
			wsprintf(str, awch, lpTemplateName, (LPWSTR)__FILE__, __LINE__);
 ...
 *
 */
HWND DoModelessDialog(HINSTANCE hInstance, LPCWSTR lpTemplateName,
