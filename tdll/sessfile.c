...
BOOL sessLoadSessionStuff(const HSESSION hSession)
	...
	if (bRet)
		...
		// Initialize...
		//
		WCHAR_Fill(hhSess->achSessName, L'\0',
			sizeof(hhSess->achSessName) / sizeof(WCHAR));
		
		sfGetSessionFileName(hhSess->hSysFile, 	
			sizeof(hhSess->achSessName) / sizeof(WCHAR), hhSess->achSessName);
...
//						 hhSess->achSessName);
		...
		hhSess->achSessName[(sizeof(hhSess->achSessName) /
				sizeof(hhSess->achSessName[0])) - 1] = L'\0';
	...
	//
	if (hhSess->achSessCmdLn[0] == L'\0' && IsWindow(hhSess->hwndSess))
		{
...
//	sfPutSessionItem(hhSess->hSysFile,
//				 SFID_SESS_NAME,
//				 (StrCharGetByteCount(hhSess->achSessName) + 1) * sizeof(WCHAR),
//				 hhSess->achSessName);
...
BOOL sessSaveBackScroll(const HSESSION hSession)
	...
	if (pszData != (ECHAR *)0)
		...
		pszPtr = pszData;
		pszEnd = pszPtr;
		while (*pszPtr != ECHAR_CAST('\0'))
			{
			if (*pszPtr != ECHAR_CAST('\r'))
				pszEnd = pszPtr;
				pszPtr++;
				}
			pszEnd = pszPtr;
...
BOOL sessRestoreBackScroll(const HSESSION hSession)
	...
	if (lSize > 0)
		{
		cchData = (size_t)lSize / sizeof(ECHAR);
		pszData = (ECHAR*)malloc((size_t)lSize + sizeof(ECHAR));
		if (pszData == NULL)
			{
			assert(pszData);
			return FALSE;
		...
		memset(pszData, 0, (size_t)lSize + sizeof(ECHAR));
		...
		sfGetSessionItem(sessQuerySysFileHdl(hSession),
						...
						pszData);
		pszPtr = pszData;
		pszLimit = pszData + cchData;
		while ((pszPtr < pszLimit) && (*pszPtr != ECHAR_CAST('\0')))
			{
			pszEnd = pszPtr;
			while ((pszEnd < pszLimit) && (*pszEnd != ECHAR_CAST('\0')) &&
					(*pszEnd != ECHAR_CAST('\r')))
				pszEnd++;
			...
			pszPtr = pszEnd;
			if ((pszPtr < pszLimit) && (*pszPtr == ECHAR_CAST('\r')))
				pszPtr++;
			if ((pszPtr < pszLimit) && (*pszPtr == ECHAR_CAST('\n')))
				pszPtr++;
			}
...
int sessCheckAndLoadCmdLn(const HSESSION hSession)
	...
	WCHAR*              pszTelnet = L"telnet:";
	WCHAR*              pszPort = NULL;
#endif
	WCHAR				acPath[FNAME_LEN], acName[FNAME_LEN];
	const HHSESSION 	hhSess = VerifySessionHandle(hSession);

	if (hhSess->achSessCmdLn[0] == L'\0')
		return -1;
	...
	for (pszStr = hhSess->achSessCmdLn;
		(*pszStr != L'\0' && nIdx < sizeof(acPath) / sizeof(WCHAR));
		...
		if (*pszStr == L'/')
			...
			if (FALSE)
				{
				MemCopy(&acPath[nIdx], pszStr, (size_t)2 * sizeof(WCHAR));
				nIdx += 2;
				}
			else
				{
	...
	//
	acPath[nIdx] = L'\0';
    WCHAR_Trim(acPath);     // Strip leading spaces
...
#endif
	GetFileNameFromCmdLine(acPath, acName, sizeof(acName) / sizeof(WCHAR));
    ...
    if (acName[0] == 0)
        ...
        if (fTestOpenOldTrmFile(hhSess, acName) != 0)
            {
            // Next, try the more common UltraTerminal file format
            //
            if (sfOpenSessionFile(hhSess->hSysFile, acName) < SF_OK)
                ...
                {
                // Command-line argument is an existing UltraTerminal file
                //
                hhSess->iCmdLnDial = CMDLN_DIAL_DIAL;
                iRet = 0;
                }
 ...
 *
 */
int fTestOpenOldTrmFile(const HHSESSION hhSess, WCHAR *pachName)
	{
	int iRet = -1;
	HANDLE hFile;
	DWORD  dw;
	LPWSTR pszPtr;
	WCHAR  ach[80];
	WCHAR  achName[FNAME_LEN];

	StrCharCopyN(achName, pachName, sizeof(achName) / sizeof(WCHAR));
	pszPtr = StrCharFindLast(achName, L'.');

	if (pszPtr && (StrCharCmpi(pszPtr, L".TRM") == 0))
		{
		/* Old .TRM files case */
		...
		if (hFile != INVALID_HANDLE_VALUE)
			...
			if (SetFilePointer(hFile, 0x282, 0, FILE_BEGIN) != (DWORD)-1)
				...
				if (ReadFile(hFile, ach, sizeof(ach), &dw, 0) == TRUE)
					{
					CloseHandle(hFile);
					ach[sizeof(ach)/sizeof(WCHAR)-1] = L'\0';
