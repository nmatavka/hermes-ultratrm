...
//#if 0 //mpt:3-10-98 for some reason, using this call causes an access violation
		...
		case WM_MENUSELECT:
			...
			if (HIWORD(wPar) & MF_POPUP)
				{
				SendMessage(sessQueryHwndStatusbar(hSession), SBR_NTFY_NOPARTS, 0, (LPARAM)(LPWSTR)"");
				return 0;
				}
			if (LOWORD(wPar))
				{
				...
				SendMessage(sessQueryHwndStatusbar(hSession), SBR_NTFY_NOPARTS, 0, (LPARAM)(LPWSTR)ach);
				}
			}
			return 0;
		...
		case WM_SESS_SHOW_SIDEBAR:
			// Autoloads can't do this directly or they hang ultraterminal.
			//
			hSession = (HSESSION)GetWindowLongPtr(hwnd, GWLP_USERDATA);
...
STATIC_FUNC LRESULT SP_WM_CMD(const HWND hwnd, const int nId, const int nNotify,
	...
	void		   *pv;
	LPWSTR		   pszFileName;
    WCHAR          ach[100], achList[100], achDir[_MAX_PATH];
#if defined(INCL_USE_HTML_HELP)
    WCHAR          achHtmlFilename[100];
#endif
	...
	switch (nId)
		...
		case IDM_ACTIONS_SEND_TEXT:
			...
			resLoadFileMask(glblQueryDllHinst(), IDS_CPF_FILES1, 2, achList,
				sizeof(achList) / sizeof(WCHAR));

			//Changed to use working folder rather than current folder - mpt 8-18-99
            if ( !GetWorkingDirectory( achDir, sizeof(achDir) / sizeof(WCHAR)) )
				{
				GetCurrentDirectory(sizeof(achDir) / sizeof(WCHAR), achDir);
				}
...
//#if 0 //mpt:3-10-98 for some reason, using this call causes an access violation
	  //            in UltraTerminal.dll. Using the winhelp call gives us the same results.
        ...
        case IDM_DISCUSSION:
            ShellExecute(NULL, "open", L"http://www.hilgraeve.com/discuss", NULL, NULL, SW_SHOW);
            break;
#endif		
        case IDM_ABOUT:
			AboutDlg(hwnd);
		...
		case IDM_CONTEXT_WHATS_THIS:
			WinHelp(hwnd, glblQueryHelpFileName(), HELP_CONTEXTPOPUP,
				(DWORD)(LPWSTR)IDH_TERM_CONTEXT_WHATS_THIS);
			break;
		#endif
