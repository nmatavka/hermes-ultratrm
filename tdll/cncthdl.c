 ...
 *
 */
int cnctSetDevice(const HCNCT hCnct, const LPWSTR pachDevice)
	{
	int iRet;
	const HHCNCT hhCnct = (HHCNCT)hCnct;
...
static int cnctLoadDriver(const HHCNCT hhCnct)
	...
	hhCnct->pfSetDestination = 
		(int (WINAPI *)(const HDRIVER, WCHAR *const, const size_t))
			cnctdrvSetDestination;

	hhCnct->pfGetComSettingsString = (int (WINAPI *)(const HDRIVER,
		LPWSTR pachStr, const size_t cb))cnctdrvGetComSettingsString;
 ...
 *
 */
static int cnctLoadDriver(const HHCNCT hhCnct, const LPWSTR pachDllName)
	{
	#define LOADPROC(x,y) \
...
int cnctIsModemConnection(const HCNCT hCnct)
    ...
    // or DIRECT_COMWINSOCK/DIRECT_COMSSH)
    //
...
void cnctMessage(const HCNCT hCnct, const int idMsg)
	...
	if (LoadString(glblQueryDllHinst(), idMsg, achFormat,
		sizeof(achFormat) / sizeof(WCHAR)) == 0)
		{
		assert(FALSE);
		return;
		}
	...
	//
 	nSize = GetTimeFormatW(lcId, 0, NULL, NULL, NULL, 0);
 	pachTime = malloc((unsigned int)(nSize+1) * sizeof(WCHAR));
 	WCHAR_Fill(pachTime, L'\0', (unsigned int)(nSize+1));

	GetLocalTime(&stSysTimeDate);
	GetTimeFormatW(lcId, 0, &stSysTimeDate, NULL, pachTime, nSize+1);

	// NOTE: The 2nd parameter to GetDateFormatW() should be DATE_LONGDATE but
	// right now that causes the function to return garbage, so for now use
	// what works!
	//
	nSize = GetDateFormatW(lcId, 0, NULL, NULL, NULL, 0);
 	pachDate = malloc((unsigned int)(nSize+1) * sizeof(WCHAR));
 	WCHAR_Fill(pachDate, L'\0', (unsigned int)(nSize+1));

	GetDateFormatW(lcId, 0, &stSysTimeDate, NULL, pachDate, nSize+1);
	...
	FormatMessage(FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY,
				achFormat, 0, 0, ach, sizeof(ach) / sizeof(WCHAR), acPtrs);
 ...
 *
 */
int	cnctSetDestination(const HCNCT hCnct, WCHAR *const ach, const size_t cb)
	{
	int iRet;
	const HHCNCT hhCnct = (HHCNCT)hCnct;
 ...
 *
 */
int cnctGetComSettingsString(const HCNCT hCnct, LPWSTR pach, const size_t cb)
	{
	int iRet;
	const HHCNCT hhCnct = (HHCNCT)hCnct;
