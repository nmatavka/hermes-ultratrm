 ...
 *
 */
int xfrSendAddToList(HXFER hXfer, LPCWSTR pszFile)
	{
	int nRet = 0;
	XD_TYPE *pX;
	LPWSTR pszName;
	...
	if (pX)
		...
		if (pszFile == NULL)
			...
			if (pX->nSendListCount == 0)
				...
				pX->acSendNames = malloc(sizeof(WCHAR *) * LIST_CHUNK);
				...
				pX->acSendNames = realloc(pX->acSendNames,
										sizeof(WCHAR *) * (pX->nSendListCount + 1 + LIST_CHUNK));
				if (pX->acSendNames == NULL)
					{
					nRet = XFR_NO_MEMORY;
					pX->nSendListCount = 0;
 ...
 *
 */
int xfrRecvStart(HXFER hXfer, LPCWSTR pszDir, LPCWSTR pszName)
	{
	HSESSION hSession;
	int nRet = 0;
	XD_TYPE *pX;
...
static void xfrInitDspStruct(HXFER hXfer)
	...
	if (pX)
		...
		WCHAR_Fill(pX->acMessage, L'\0',
					sizeof(pX->acMessage) / sizeof(WCHAR));
		WCHAR_Fill(pX->acOurName, L'\0',
					sizeof(pX->acOurName) / sizeof(WCHAR));
		WCHAR_Fill(pX->acTheirName, L'\0',
					sizeof(pX->acTheirName) / sizeof(WCHAR));
		}
	}
    ...
    if (pX)
        ...
        switch (pX->nDirection)
            ...
            case XFER_SEND:
                ...
                // Fix bug 441193, during file transfer, the tool bar
                // buttons for send/rcv must be disabled
                hwndToolbar = sessQueryHwndToolbar(hSession);

                ToolbarEnableButton (hwndToolbar, IDM_ACTIONS_SEND, FALSE);
                ToolbarEnableButton (hwndToolbar, IDM_ACTIONS_RCV,  FALSE);

                nRet = xfrSend(hSession, (XFR_SEND *)pX->pXferStuff);

                ToolbarEnableButton (hwndToolbar, IDM_ACTIONS_SEND, TRUE);
                ToolbarEnableButton (hwndToolbar, IDM_ACTIONS_RCV,  TRUE);
