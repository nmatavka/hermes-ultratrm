...
int CLoopInitHdl(const HCLOOP hCLoop)
	...
	pstCLoop->stWorkSettings.chWaitChar 	   = L'\r';
...
int CLoopLoadHdl(const HCLOOP hCLoop)
	...
	ul = sizeof(pstCLoop->fDoHostWide);
	sfGetSessionItem(sfhdl, SF_CLOOP_HOSTWIDTH, &ul, &pstCLoop->fDoHostWide);
...
int CLoopSaveHdl(const HCLOOP hCLoop)
	...
	if (memcmp(&pstCLoop->stWorkSettings, &pstCLoop->stOrigSettings,
		...
		PutItem(pstCLoop->fDoHostWide, SF_CLOOP_HOSTWIDTH);
...
WCHAR CLoopGetWaitChar(const HCLOOP hCLoop)
	{
	WCHAR chVal;
...
void CLoopSetWaitChar(const HCLOOP hCLoop, WCHAR chWaitChar)
	{
	assert(hCLoop);
	EnterCriticalSection(&((ST_CLOOP *)hCLoop)->csect);
