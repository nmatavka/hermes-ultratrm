...
STATIC_FUNC BOOL asCreateFullFileName(const HSESSION hSession,
									  ...
									  LPWSTR acFile,
									  BOOL fExplicit);
STATIC_FUNC int  asOverwriteExistingFile(HWND hwnd, LPWSTR pacName);
STATIC_FUNC void asBadSessionFileMsg(HSESSION hSession, LPWSTR pacName);
...
void SilentSaveSession(const HSESSION hSession, HWND hwnd, BOOL fExplicit)
	...
	if (fExplicit == 0)
		{
		acName[0] = L'\0';
		sessQueryName(hSession, acName, sizeof(acName) / sizeof(WCHAR));
	...
	sfGetSessionFileName(hhSess->hSysFile,
							sizeof(acName) / sizeof(WCHAR), acName);
	...
	// qualified file name.
	//
	if (acName[0] == L'\0' || fNameChanged)
		{
		if (!asCreateFullFileName(hSession,
				sizeof(acName) / sizeof(WCHAR), acName, fExplicit))
			{
	...
	if (hhSess->fIsNewSession)
		...
		{
		if (acOldFile[0] != L'\0' && fNameChanged)
			{
...
BOOL SaveSession(const HSESSION hSession, HWND hwnd)
	...
	if (sessQueryIsNewSession(hSession) == TRUE)
		{
		WCHAR_Fill(achSessName, L'\0', sizeof(achSessName) / sizeof(WCHAR));
		WCHAR_Fill(achText, L'\0', sizeof(achText) / sizeof(WCHAR));

		LoadString(glblQueryDllHinst(), IDS_GNRL_CNFRM_SAVE, achText,
			sizeof(achText) / sizeof(WCHAR));
		...
		wsprintf(ach, achText, L"\"", achSessName, L"\"");
...
int OpenSession(const HSESSION hSession, HWND hwnd)
	...
	const HHSESSION hhSess = VerifySessionHandle(hSession);
	LPWSTR 			pszStr;
	WCHAR 			acMask[64], acTitle[64];
	WCHAR 			acDir[FNAME_LEN], acName[FNAME_LEN];
	...
	if ( IsNT() )
#endif
		GetUserDirectory(acDir, sizeof(acDir) / sizeof(WCHAR));
#ifdef NT_EDITION
	else
		{
		GetModuleFileName(glblQueryHinst(), acDir, sizeof(acDir) / sizeof(WCHAR));
		mscStripName(acDir);
		}
#endif
...
void SaveAsSession(const HSESSION hSession, HWND hwnd)
	...
	if (sfGetSessionFileName(sessQuerySysFileHdl(hSession),
			sizeof(acFileName) / sizeof(WCHAR), acFileName) != SF_OK ||
			sfGetSessionFileName(sessQuerySysFileHdl(hSession),
			sizeof(acDir) / sizeof(WCHAR),	acDir) != SF_OK)
		{
		acFileName[0] = L'\0';

		if (asCreateFullFileName(hSession,
								sizeof(acFileName) / sizeof(WCHAR),
								acFileName,
								TRUE) == TRUE)
			{
			StrCharCopyN(acDir, acFileName, sizeof(acDir) / sizeof(WCHAR));
            acDir[sizeof(acDir) / sizeof(WCHAR) - 1] = L'\0';
			mscStripName(acDir);
			}
		else
			{
			//Changed from current directory to user directory - mpt 8-18-99
			if ( !GetUserDirectory(acDir, sizeof(acDir) / sizeof(WCHAR)) )
				{
				GetCurrentDirectory(sizeof(acDir) / sizeof(WCHAR), acDir);
				}
			sessQueryName(hSession, acFileName, sizeof(acFileName) / sizeof(WCHAR));
			}
		}
    else
        {
	...
	if (pszStr)
		...
		StrCharCopyN(ach, pszStr, sizeof(ach) / sizeof(WCHAR));
		StrCharCopyN(hhSess->achSessName, mscStripExt(mscStripPath(ach)),
                     sizeof(hhSess->achSessName)/sizeof(WCHAR));
		StrCharCopyN(hhSess->achOldSessName, hhSess->achSessName,
                     sizeof(hhSess->achOldSessName)/sizeof(WCHAR));
...
STATIC_FUNC BOOL asCreateFullFileName(const HSESSION hSession,
	...
	LoadString(glblQueryDllHinst(), IDS_GNRL_HAS, acExt, sizeof(acExt) / sizeof(WCHAR));
...
STATIC_FUNC int asOverwriteExistingFile(HWND hwnd, LPWSTR pacName)
	{
	WCHAR 	ach[256], achTitle[256], achText[256];
	int 	nRet = 0;
...
STATIC_FUNC void asBadSessionFileMsg(HSESSION hSession, LPWSTR pacName)
	{
	const HHSESSION hhSess = VerifySessionHandle(hSession);
	WCHAR acFormat[64], acTitle[256], ach[256];

	WCHAR_Fill(acFormat, L'\0', sizeof(acFormat) / sizeof(WCHAR));
	WCHAR_Fill(acTitle, L'\0', sizeof(acTitle) / sizeof(WCHAR));

	LoadString(glblQueryDllHinst(), IDS_CMM_LOAD_SESS, acTitle,
		sizeof(acTitle) / sizeof(WCHAR));
	LoadString(glblQueryDllHinst(),	IDS_ER_BAD_SESSION, acFormat,
		sizeof(acFormat) / sizeof(WCHAR));
	...
	sfSetSessionFileName(hhSess->hSysFile, L"");
	}
