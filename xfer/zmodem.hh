...
struct z_mdm_cntrl
	...
	BYTE     Rxhdr[4];			/* Received header */
	BYTE     Txhdr[4];			/* Transmitted header */
	...
	BYTE     Attn[ZATTNLEN+1];	/* Attention string rx sends to tx on err */
	...
	long     Modtime;
	int      do_init;
	BYTE     zconv; 		/* ZMODEM file conversion request */
	BYTE     zmanag;		/* ZMODEM file management request */
	BYTE     ztrans;		/* ZMODEM file transport request */
	BYTE    *secbuf;
	WCHAR   *fname;
	WCHAR   *our_fname;
	...
	int	     Beenhereb4;
	BYTE	 Wantfcs32;			/* Do we want 32 bit crc			   */
	int	     Rxflags;
...
void zmdms_newfile   (ZC *zc, int filen, WCHAR *fname, long flength);
...
void zmdmr_newfile   (ZC *zc, int filen, BYTE *theirname, WCHAR *ourname);
