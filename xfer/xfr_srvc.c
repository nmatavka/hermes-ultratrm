 ...
 *
 */
void xfer_purgefile(HSESSION hSession, WCHAR *fname)
	{
 ...
 *
 */
static int xfrUniqueName(HSESSION hSession, LPWSTR pszSrc, LPWSTR pszDst)
	{
	int nRetVal = -1;
	WCHAR szSrc[_MAX_PATH];
	WCHAR szName[_MAX_PATH];
	WCHAR szTag[10];			// big enough to hold "0" to "9999"
	WCHAR *pszFName = NULL;
	...
	if (nSize)
		...
		if (pszFName)
			...
			pszExtension = StrCharFindLast(pszFName, L'.');
			}
 ...
 *
 */
static int xfrUniqueDateName(HSESSION hSession, LPWSTR pszSrc, LPWSTR pszDst)
	{
	int nRet = 0;
	int nTag;
	LPWSTR pszDir;
	...
	wsprintf(acDatestr, "%x%02d%02d%1d",
						...
						stT.wHour,
						stT.wMinute % 10);
	acFrm[8 - StrCharGetByteCount(acDatestr)] = L'\0';
 ...
 *
 */
int xfer_makepaths(HSESSION hSession, LPWSTR pszPath)
	{
	WCHAR ach[256], achFormat[256], ach2[50];
...
void xfer_build_rcv_name(HSESSION hSession,
	...
	if (nSingle)
		...
		pszStr = (LPWSTR)StrCharLast(acBuffer);
		if (*pszStr != L'\\')
			{
			pszStr += 1;
			*pszStr = L'\\';
		...
		while (pszStr >= pstRcv->pszSuggestedName)
			...
			if ((*pszStr == L'\\') || (*pszStr == L':'))
				...
				if (pszStr == pstRcv->pszSuggestedName)
					break;
				pszStr = (LPWSTR)StrCharPrev(pstRcv->pszSuggestedName, pszStr);
				}
			}
		...
		if ((pstRcv->pszSuggestedName[0] == L'\\') ||
			(pstRcv->pszSuggestedName[1] == L':'))
			{
			/* if full path given */
 ...
 *
 */
int xfer_nextfile(HSESSION hSession, WCHAR *filename)
	{
	XD_TYPE *pX;
	XFR_SEND *pS;
	LPWSTR pszStr;
...
void xfer_name_to_send(HSESSION hSession,
	 ...
	 */
	pszStr = StrCharFindLast(local_name, L'\\');
	if (*pszStr == L'\\')
		pszStr += 1;
