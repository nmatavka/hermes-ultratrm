...
VOID long_to_octal(LONG lVal, WCHAR *pszStr);
...
USHORT zmdm_snd(HSESSION h, int method, int attended, unsigned nfiles, long nbytes)
	{
	ZC           *zc;
	WCHAR	     sfname[FNAME_LEN]; // file name of file being sent
	int	         got_file = FALSE;  // controls when to complete batch op
...
/*----------------------------------------------------------------------+
 | wcs
 +----------------------------------------------------------------------*/
static void zmdm_name_to_send_bytes(HSESSION hSession,
		WCHAR *localName,
		BYTE *nameToSend,
		size_t nameToSendSize)
	{
	WCHAR wideName[FNAME_LEN];

	if (nameToSend == NULL || nameToSendSize == 0)
		return;
	...
	wideName[0] = L'\0';
	xfer_name_to_send(hSession, localName, wideName);

#if defined(ULTRATERMINAL_UNICODE_ONLY)
	if (ut_wide_to_utf8(wideName, (char *)nameToSend,
			(int)nameToSendSize) == 0)
		nameToSend[0] = '\0';
#else
	StrCharCopyN((WCHAR *)nameToSend, wideName, (int)nameToSendSize);
#endif
	}

int wcs(ZC *zc, WCHAR *oname)
	{
	int c;
	WCHAR name[PATHLEN];
...
/*----------------------------------------------------------------------+
 ...
 +----------------------------------------------------------------------*/
int wctxpn(ZC *zc, WCHAR FAR *name)
	{
	register BYTE *p, *q;
	int serial_number = 0;
	XFR_Z_PARAMS *pZ;
	...
	zmdm_name_to_send_bytes(zc->hSession, name, zc->txbuf, ZBUF_SIZE);

	q = &zc->txbuf[strlen((const char *)zc->txbuf) + 1];
	...
	// Since this character could be the second byte of a legacy double-byte character
	...
	return zsendfile(zc, (char *)zc->txbuf,
			(int)(1 + strlen((const char *)p) + (p - zc->txbuf)));
	}
