...
int    procheader (ZC *zc, BYTE *name);
...
USHORT zmdm_rcv(HSESSION h, int method, int attended, int single_file)
	...
	zc->fname = malloc(FNAME_LEN * sizeof(WCHAR));
	if (zc->fname == NULL)
		goto done;

	zc->our_fname = malloc(FNAME_LEN * sizeof(WCHAR));
...
int rzfile(ZC *zc)
	...
	if (procheader(zc, zc->secbuf) == ERROR)
		{
			...
			case ZDATA:
				if( ! fBlocking )
					...
					if( rclhdr(zc->Rxhdr) != zc->file_bytes)
						...
						if ( --n < 0)
							{
							DbgOutStr("ZMODEM error %s %d", UT_WFILE, __LINE__,0,0,0);
#if defined(DEBUG_DUMPPACKET)
...
int procheader(ZC *zc, BYTE *name)
	...
	WCHAR loc_fname[FNAME_LEN];

#if defined(ULTRATERMINAL_UNICODE_ONLY)
	if (ut_utf8_to_wide((const char *)name, zc->fname, FNAME_LEN) == 0)
		MultiByteToWideChar(CP_ACP, 0, (const char *)name, -1,
				zc->fname, FNAME_LEN);
#else
	StrCharCopy(zc->fname, (WCHAR *)name);
#endif
	CharUpper(zc->fname);
	zc->Thisbinary = FALSE;
	...
	p = (char *)name + 1 + strlen((const char *)name);
	if (*p)
		...
		for (pt = zc->fname; *pt; ++pt)	   /* change / to _ */
			if ( *pt == L'/')
				*pt = L'_';

		if (pt > zc->fname && *--pt == L'.')		/* zap trailing period */
			*pt = 0;
		}
	...
	if (file_err != 0)
		...
		zc->filen += 1;
		zmdmr_newfile (zc, zc->filen, name, stRcv.pszActualName);
		return ERROR;
		}
	else
		{
