...
static WCHAR szHelpFileName[FNAME_LEN];
 ...
 *
 */
static void transPumpOutString(WCHAR *cReady, int *nReady, WCHAR *cOut)
	{
	WCHAR *pszPtr;
...
static void transPumpOutChar(WCHAR *cReady, int *nReady, WCHAR cOut)
	{
	WCHAR *pszPtr;
...
static void transShiftToJisOut(struct stShiftToJis *pstS,
	...
	cOne = (WCHAR)(((cOne - rowOffset) << 1) - adjust);
	cTwo -= (WCHAR)cellOffset;
...
static int transHalfToFullKatakana(struct stShiftToJis *pstS,
	...
	cOne = (WCHAR)mtable[tmp - 161][0];
	cTwo = (WCHAR)mtable[tmp - 161][1];
		...
		switch(pstI->nInputMode)
			...
			case JIS_TO_SHIFT_MODE:
				...
				if (cIn == L'\x1B')
					...
					if (!nOK)
						{
						pstJ->acBuffer[pstJ->nEscSeqCount] = L'\0';
						/*
						 * Dump out whatever it is we have seen
						 */
						if (pstJ->nSeenEscape)
							{
							transPumpOutChar(cReady, nReady, L'\x1B');
							}
						transPumpOutString(cReady,
											nReady,
											pstJ->acBuffer);
		...
		switch(pstI->nOutputMode)
			...
			case SHIFT_TO_JIS_MODE:
				...
				if ((cOut == L'\r') || (cOut == L'\n'))
					...
					{
					/* If the character is a legacy double-byte lead byte */
			...
			case SHIFT_TO_EUC_MODE:
				...
				if (pstS->nLeadByteSeen)
					...
					{
					/* If the character is a half width katakana character */
					transPumpOutChar(cReady, nReady, (WCHAR)0x8E);
					transPumpOutChar(cReady, nReady, cOut);
					}
				else
					{
...
INT_PTR CALLBACK EncodeSelectDlg(HWND hDlg, UINT wMsg, WPARAM wPar, LPARAM lPar)
	...
	case WM_CONTEXTMENU:
		...
		LoadString(hInstanceDll, IDS_GNRL_HELPFILE, szHelpFileName,
					sizeof(szHelpFileName) / sizeof(WCHAR));

		WinHelp((HWND)wPar, szHelpFileName, HELP_CONTEXTMENU,
			 (DWORD_PTR)(LPWSTR)aHlpTable);
#endif
		break;
	...
	case WM_HELP:
		...
		WinHelp(((LPHELPINFO)lPar)->hItemHandle, szHelpFileName,
			HELP_WM_HELP, (DWORD_PTR)(LPWSTR)aHlpTable);
