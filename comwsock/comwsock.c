 ...
 * Fix for setting character in a WCHAR array to correct NULL character.
 * 
 * Fix for setting character in a WCHAR array to correct NULL character.
 * 
	...
	if (StrCharCmpi(pszCommand, L"SET") == 0)
		...
		if (*pszItem)
			...
			switch(iIndex)
				{
			case 0: // IPADDR
				StrCharCopyN(szResult, pstPrivate->szRemoteAddr, (sizeof(szResult)/sizeof(WCHAR)));
...
static int WinSockIsVideotex(ST_STDCOM *pstPrivate)
	...
	return (hEmu && emuQueryEmulatorId(hEmu) == EMU_VIDEOTEX) ? TRUE : FALSE;
	}
#endif
