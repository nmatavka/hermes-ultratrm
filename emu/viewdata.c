...
void EmuViewdataAnswerback(const HHEMU hhEmu)
	...
	if (StrCharGetStrLength(sp) == 0)
        ...
        pech = malloc((unsigned int)StrCharGetByteCount(sp) + sizeof(WCHAR));
	...
	if (pech == NULL)
        ...
        CnvrtHostToECHAR(pech, (unsigned long)StrCharGetByteCount(sp), sp,
                     (unsigned long)StrCharGetByteCount(sp) + sizeof(WCHAR));
 ...
 *
 * DESCRIPTION: Displays the last displayed mosaic WCHAR inn every attribute
...
void EmuViewdataSetAttr(const HHEMU hhEmu)
	...
	if (uch >= ECHAR_CAST('\x41') && uch <= ECHAR_CAST('\x47'))  // A thru G
		{
		stAttr[iRow][iCol].attr = ALPHA_ATTR;
		stAttr[iRow][iCol].clr	= aiColors[uch - ECHAR_CAST('\x41')];
		}

	else if (uch >= ECHAR_CAST('\x51') && uch <= ECHAR_CAST('\x57')) // Q thru W
		{
		charattr = GetAttr(hhEmu, hhEmu->emu_currow, hhEmu->emu_curcol);
		stAttr[iRow][iCol].attr = MOSAIC_ATTR;
		stAttr[iRow][iCol].clr	= aiColors[uch - ECHAR_CAST('\x51')];
		}
...
void EmuViewdataCharDisplay(const HHEMU hhEmu)
	...
	if (hhEmu->emu_code == ECHAR_CAST('\x7F'))
		hhEmu->emu_code = ECHAR_CAST('\x5B');
...
int EmuViewdataKbd(const HHEMU hhEmu, int kcode, const BOOL fTest)
	...
	case VK_ESCAPE | VIRTUAL_KEY:
		kcode = ECHAR_CAST('[') | CTRL_KEY;
		if (fTest)
			return kcode;
		break;

	case VK_TAB | VIRTUAL_KEY:
		kcode = ECHAR_CAST('I') | CTRL_KEY;
		if (fTest)
			return kcode;
		break;

	case VK_RETURN | VIRTUAL_KEY | CTRL_KEY:
		kcode = ECHAR_CAST('J') | CTRL_KEY;
		if (fTest)
			return kcode;
		break;
	default:
