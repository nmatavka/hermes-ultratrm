...
typedef struct
	{
	int key;
	int fPointer;	// TRUE means we use the BYTE * portion of the union.
	int uLen;		// length of stored keystring.
	union
		{
		BYTE	achKeyStr[sizeof(void *)];
		BYTE   *pachKeyStr;
		} u;
	} KEY;
...
struct stEmuInternal
	...
	// Is the loaded emulator legacy double-byte Enabled
	int	fHostWideSupported;			/* Emulator is legacy double-byte Enabled	True\False. */

	// JFH:2/22/95 WCHAR	dspchar[256];		/* Character display map */
