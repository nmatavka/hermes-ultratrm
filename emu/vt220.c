...
void vt220_DA(const HHEMU hhEmu)
	...
	//
	if (hhEmu->mode_vt320)
		StrCharCat(achStr, L"?63");
	else
		StrCharCat(achStr, L"?62");

	// Add the rest of the respnse and send the result.
	//
	StrCharCat(achStr, L";1;2;6;8;9;14c");

	CnvrtHostToECHAR(ech, sizeof(ech), achStr, 
...
void vt220_2ndDA(const HHEMU hhEmu)
	...
	if (hhEmu->mode_vt320)
        {
		StrCharCat(achStr, L">24;14;0c");
        }
	else
        {
		StrCharCat(achStr, L">1;23;0c");
        }
...
void emuDecDefineUDK(const HHEMU hhEmu)
	...
	WCHAR acAscii[16] = {   L'0', L'1', L'2', L'3',
							L'4', L'5', L'6', L'7',
							L'8', L'9', L'A', L'B',
							L'C', L'D', L'E', L'F' };
	...
	case(KEY_NUMBER_NEXT):
		WCHAR_Fill(pstPRI->acUDKSequence,
			0, sizeof(pstPRI->acUDKSequence) / sizeof(WCHAR));

		pstPRI->iUDKSequenceLen = 0;
		pstPRI->chUDKAssignment = 0;
		pstPRI->iUDKState		= KEY_DIGIT2_NEXT;

		if (IN_RANGE(emuCode,L'1',L'3'))
			{
			...
			pstPRI->chUDKAssignment = (WCHAR)(aiHex[idx] << 4);
	...
	case(KEY_DIGIT2_NEXT):
		if (isdigit(emuCode))
			...
			pstPRI->chUDKAssignment += (WCHAR)aiHex[idx];
...
int emuDecStoreUDK(const HHEMU hhEmu)
	...
	//
	pstKey->pSequence = malloc( sizeof(WCHAR) *
...
void emuDecSendKeyString(const HHEMU hhEmu, const int iIndex,
    ...
    case 0x84:
        // Send Esc - D
        //
	        pchLeadChar = (const BYTE *)"\x1B\x44";
	        break;

    case 0x8F:
        // Send Esc - O
        //
	        pchLeadChar = (const BYTE *)"\x1B\x4F";
	        break;

    case 0x9B:
        // Send Esc - [
        //
	        pchLeadChar = (const BYTE *)"\x1B\x5B";
	        break;
    ...
    //
	    if (pchLeadChar != NULL)
	        {
			CLoopBytesOut(sessQueryCLoopHdl(hhEmu->hSession), pchLeadChar, 2);
			if (pstKeyData->iSequenceLen > 1)
				CLoopBytesOut(sessQueryCLoopHdl(hhEmu->hSession),
						pstKeyData->pSequence + 1,
						(int)pstKeyData->iSequenceLen - 1);
	        }
    else
        {
        emuVT220SendKeyString(hhEmu, iIndex, pstcKeyTbl, iMaxEntries);
...
void emuVT220SendKeyString(const HHEMU hhEmu,
    ...
    DbgOutStr("%s", pstcKeyTbl[iIndex].pszKeyName, 0, 0, 0, 0);
	    ...
	    if (pstKeyData->iSequenceLen > 0)
	        {
			CLoopBytesOut(sessQueryCLoopHdl(hhEmu->hSession),
					pstKeyData->pSequence,
					(int)pstKeyData->iSequenceLen);
	        }
    }
