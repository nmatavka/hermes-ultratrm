...
int emuInitializeHdl(const HEMU hEmu)
    ...
    //
    stBaseSFSettings.acTelnetId[0]		= L'\0';
...
#ifdef INCL_USER_DEFINED_BACKSPACE_AND_TELNET_TERMINAL_ID
        if ( stBaseSFSettings.acTelnetId[0] == L'\0' )
            {
            emuQueryDefaultTelnetId(stBaseSFSettings.nEmuId,
					stBaseSFSettings.acTelnetId, EMU_MAX_TELNETID);
            }
	...
	StrCharCopy(hhEmu->stUserSettings.acVideotexProfile, L"NXTel");
	hhEmu->stUserSettings.acVideotexHost[0] = L'\0';
	hhEmu->stUserSettings.nVideotexPort = 0;
	hhEmu->stUserSettings.nVideotexFontMap = 0;
	hhEmu->stUserSettings.fVideotexTelnetCompat = FALSE;

	if ((nRet = emuLoadSettings(hhEmu,
						SFID_EMU_VIDEOTEX_PROFILE,
						sizeof(hhEmu->stUserSettings.acVideotexProfile),
						hhEmu->stUserSettings.acVideotexProfile)) == -1)
		{
		nReturn = -1;
		goto InitExit;
		}
	if ((nRet = emuLoadSettings(hhEmu,
						SFID_EMU_VIDEOTEX_HOST,
						sizeof(hhEmu->stUserSettings.acVideotexHost),
						hhEmu->stUserSettings.acVideotexHost)) == -1)
		{
		nReturn = -1;
		goto InitExit;
		}
	if ((nRet = emuLoadSettings(hhEmu,
						SFID_EMU_VIDEOTEX_PORT,
						sizeof(hhEmu->stUserSettings.nVideotexPort),
						&hhEmu->stUserSettings.nVideotexPort)) == -1)
		{
		nReturn = -1;
		goto InitExit;
		}
	if ((nRet = emuLoadSettings(hhEmu,
						SFID_EMU_VIDEOTEX_FONT,
						sizeof(hhEmu->stUserSettings.nVideotexFontMap),
						&hhEmu->stUserSettings.nVideotexFontMap)) == -1)
		{
		nReturn = -1;
		goto InitExit;
		}
	if ((nRet = emuLoadSettings(hhEmu,
						SFID_EMU_VIDEOTEX_TELNET,
						sizeof(hhEmu->stUserSettings.fVideotexTelnetCompat),
						&hhEmu->stUserSettings.fVideotexTelnetCompat)) == -1)
		{
		nReturn = -1;
		goto InitExit;
		}
...
int emuSaveHdl(const HEMU hEmu)
	...
	sfPutSessionItem(sessQuerySysFileHdl(hhEmu->hSession),
						SFID_EMU_VIDEOTEX_PORT,
						sizeof(hhEmu->stUserSettings.nVideotexPort),
						&hhEmu->stUserSettings.nVideotexPort);
	sfPutSessionItem(sessQuerySysFileHdl(hhEmu->hSession),
						SFID_EMU_VIDEOTEX_FONT,
						sizeof(hhEmu->stUserSettings.nVideotexFontMap),
						&hhEmu->stUserSettings.nVideotexFontMap);
	sfPutSessionItem(sessQuerySysFileHdl(hhEmu->hSession),
						SFID_EMU_VIDEOTEX_TELNET,
						sizeof(hhEmu->stUserSettings.fVideotexTelnetCompat),
						&hhEmu->stUserSettings.fVideotexTelnetCompat);
#endif
...
int emuQueryName(const HEMU hEmu, WCHAR *achBuffer, int nSize)
	...
	for (indx = 0 ; indx < nEmuCount ; indx++)
		{
		nLen = StrCharGetByteCount((LPWSTR)pv) + (int)sizeof(BYTE);
...
#if defined(INCL_VIDEOTEX)
		case EMU_VIDEOTEX:
			nResourceId = IDS_EMUNAME_VIDEOTEX;
			break;
#endif
...
#if defined(INCL_VIDEOTEX)
	if (pstSettings->nVideotexPort < 0 || pstSettings->nVideotexPort > 65535)
		{
		pstSettings->nVideotexPort = 0;
		iReturn = -1;
		}
	if (pstSettings->nVideotexFontMap < 0 || pstSettings->nVideotexFontMap > 1)
		{
		pstSettings->nVideotexFontMap = 0;
		iReturn = -1;
		}
#endif
...
int emuStdDataIn(const HHEMU hhEmu, const ECHAR ccode)
	...
	hhEmu->emu_code = ECHAR_CAST(ccode);
	...
	if (hhEmu->state == 0)
		{
		if (IN_RANGE(ccode, ECHAR_CAST(' '), hhEmu->emu_highchar) ||
						ccode == ECHAR_CAST('\r') ||
						ccode == ECHAR_CAST('\n'))
			{
			CaptureChar(sessQueryCaptureFileHdl(hhEmu->hSession),
...
#if defined(INCL_VT220)
    case EMU_VT220:
        pszDefaultId = L"VT220";
        break;
#endif

#if defined(INCL_VT320)
    case EMU_VT320:
        pszDefaultId = L"VT320";
        break;
#endif

#if defined(INCL_VT100PLUS)
    case EMU_VT100PLUS:
        pszDefaultId = L"VT100+";
        break;
#endif

#if defined(INCL_VTUTF8)
    case EMU_VTUTF8:
        pszDefaultId = L"VT-UTF8";
        break;
#endif
...
STATIC_FUNC void emuCheckSettings(struct stEmuBaseSFSettings *pstBaseSFSettings) 
	...
	if ((pstBaseSFSettings->nBackspaceKeys != EMU_BKSPKEYS_CTRLH) &&
		...
		psz = (WCHAR *)&pstBaseSFSettings->fAllowUserKeys;
	...
	if ((pstBaseSFSettings->nBackspaceKeys != EMU_BKSPKEYS_CTRLH) &&
	    ...
	    pstBaseSFSettings->acTelnetId[0] = L'\0';
		}
	}
