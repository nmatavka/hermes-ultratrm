...
void ANSI_DSR(const HHEMU hhEmu)
	...
	if (sel == 5)
		CnvrtHostToECHAR(s, sizeof(s), L"\033[0n",
                         StrCharGetByteCount(L"\033[0n"));

	else if (sel == 6)
		{
		wsprintf(achTemp, L"\033[%d;%dR", hhEmu->emu_currow + 1,
				 (hhEmu->emu_curcol == hhEmu->emu_maxcol + 1) ?
				 hhEmu->emu_curcol : hhEmu->emu_curcol + 1);
		CnvrtHostToECHAR(s, sizeof(s), achTemp, StrCharGetByteCount(achTemp));
		}

			/* 1st str is printer not ready */
	else if (sel == 0x0F25) /* are user-defined keys locked? */
		CnvrtHostToECHAR(s, sizeof(s), L"\033[?20n",
                         StrCharGetByteCount(L"\033[?20n"));

	else if (sel == 0x0F26) /* what is the keyboard language? */
		CnvrtHostToECHAR(s, sizeof(s), L"\033[?27;1n",
                         StrCharGetByteCount(L"\033[?27;1n"));
...
void vt_alt_kpmode(const HHEMU hhEmu)
	...
	else if (hhEmu->emu_code == ECHAR_CAST('>'))
...
void vt_screen_adjust(const HHEMU hhEmu)
	...
	for (row = 0; row < MAX_EMUROWS; ++row)
		{
		ECHAR_Fill(hhEmu->emu_apText[row], L'E', (unsigned)MAX_EMUCOLS);
...
void vt_DCH(const HHEMU hhEmu)
	...
	if (nchars < 1)
		nchars = 1;
	if (hhEmu->emu_code == ECHAR_CAST('P'))	 /* delete & shift line left */
		{
		ANSI_DCH(hhEmu);
		}
