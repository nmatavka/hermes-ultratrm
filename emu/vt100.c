...
void ANSI_DA(const HHEMU hhEmu)
	...
	CnvrtHostToECHAR(ech, sizeof(ech), L"\033[?1;2c", 
					 StrCharGetByteCount(L"\033[?1;2c"));
...
void vt100_savecursor(const HHEMU hhEmu)
	...
	if (sel == ECHAR_CAST('7')) /* save cursor & attributes */
		{
	...
	else if (sel == ECHAR_CAST('8'))	  /* restore cursor and attr. */
		{
...
void vt100_answerback(const HHEMU hhEmu)
	...
	CnvrtHostToECHAR(pech, StrCharGetByteCount(sp), sp, StrCharGetByteCount(sp));
...
void vt100_report(const HHEMU hhEmu)
	...
	wsprintf(str, L"\x01B[%s;",
				sol == 0 ? (LPWSTR)L"2" : (LPWSTR)L"3");
	...
	// Data bits
	//
	nDataBits == 8 ? StrCharCat(str, L"1;") : StrCharCat(str, L"2;");
	...
	//
	StrCharCat(str, L";1;0x");

	sp = str;

	CnvrtHostToECHAR(ech, sizeof(ech), sp, StrCharGetByteCount(sp));
...
#if FALSE	// Never used in UltraTerminal.
/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
 * fakevt_kbdin
 *
 * DESCRIPTION:
	...
	switch (sel)
		...
		case ECHAR_CAST(0x00):
			...
			if (hhEmu->mode_DECPFF == SET)	 /* print form feed */
				{
				CnvrtHostToECHAR(aechBuf, sizeof(aechBuf), L"0x0C",
                                 StrCharGetByteCount(L"0x0C"));
...
void vt100_prnc(const HHEMU hhEmu)
	...
	*pstPRI->pntr = 0;
	...
	CnvrtHostToECHAR(aechBuf, sizeof(aechBuf), L"\033[4i",
                     StrCharGetByteCount(L"\033[4i"));

	size = (size_t)StrCharGetByteCount(L"\033[4i");
	if ((pstPRI->len_s >= 4) && (memcmp(pstPRI->pntr - 4, aechBuf, size) == 0))
		...
		if( !hhEmu->stUserSettings.fPrintRaw)
			{
			printEchoRaw(hhEmu->hPrintHost, pstPRI->storage, pstPRI->len_s - ((*(pstPRI->pntr - 3) == (WCHAR)L'\233') ? 3 : 4));
			}
		else
#endif
			{
    		emuPrintChars(hhEmu, pstPRI->storage, pstPRI->len_s - ((*(pstPRI->pntr - 3) == (WCHAR)L'\233') ? 3 : 4));
	    	printEchoChar(hhEmu->hPrintHost, ECHAR_CAST('\n'));
			}
...
void emuSetDoubleAttrRow(const HHEMU hhEmu, const int iLineAttr)
	...
	//
	hhEmu->emu_apText[iImgRow][hhEmu->emu_maxcol + 1] = ECHAR_CAST('\0');
...
void emuFromDblToSingle(const HHEMU hhEmu)
	...
	while (iSource <= hhEmu->emu_maxcol)
		{
		if (hhEmu->emu_apText[iImgRow][iSource] == ECHAR_CAST('\0'))  // TODO 1:
			hhEmu->emu_apText[iImgRow][iDest] = ECHAR_CAST(' ');
		else
	...
	for (i = iLastCol; i < MAX_EMUCOLS ; ++i)
		{
		hhEmu->emu_apText[iImgRow][i] = ECHAR_CAST(' ');		  // TODO 2:
