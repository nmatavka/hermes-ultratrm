...
void vt100_prnc(void)
	{
#if 0
	static WCHAR storage[40];
	static WCHAR *pntr = storage;
	static int len_s = 0;
	...
	*pntr = L'\0';
	...
	if ((cnfg.emu_term == EMU_VT220 && len_s>=3 && strcmp(pntr - 3, L"\2334i") == 0)
			|| (len_s >= 4 && lstrcmp(pntr - 4, L"\033[4i") == 0))
		{
		/* received termination string, wrap it up */
		emu_print(storage, len_s - ((*(pntr - 3) == (WCHAR)L'\233') ?
					 3 : 4));
		pntr = storage;
		len_s = 0;
