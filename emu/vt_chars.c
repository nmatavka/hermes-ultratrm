    ...
    void vt_charset_init(const HHEMU hhEmu)
    	...
    	if (hhEmu->mode_vt220 || hhEmu->mode_vt320)
    		...
    		{
    		pstPRI->vt_charset[0] = pstPRI->vt_charset[1] = ECHAR_CAST('B');	  /* ASCII char set */
    		pstPRI->vt_charset[2] = pstPRI->vt_charset[3] = ECHAR_CAST('<');	  /* DEC supplemental char set */
    		pstPRI->gl = 0;
    		pstPRI->gr = 2;
    		}
    		...
    		{
    		pstPRI->vt_charset[0] = ECHAR_CAST('B');	  /* ASCII char set */
    		pstPRI->vt_charset[1] = ECHAR_CAST('B');	  /* ASCII char set */
    		pstPRI->vt_charset[2] = ECHAR_CAST('B');	  /* ASCII char set */
    		pstPRI->vt_charset[3] = ECHAR_CAST('0');	  /* DEC special graphics set */
    		pstPRI->gl = 0;
    		pstPRI->gr = 2;
    		}
    	else
    		{
    		pstPRI->vt_charset[0] = ECHAR_CAST('B');	  /* ASCII char set */
    		pstPRI->vt_charset[1] = ECHAR_CAST('0');	  /* DEC special graphics set */
    		pstPRI->gl = 0;
    		pstPRI->gr = 1;
    		}
    ...
    void vt_setkanji(const HHEMU hhEmu, const int nSetKanji)
    	...
    	if (nSetKanji)
    		...
    		pstPRI->vt_charset[1] = ECHAR_CAST('0');	  /* DEC special graphics set */
    		pstPRI->vt_charset[2] = ECHAR_CAST('B');	  /* ASCII char set */
    		pstPRI->vt_charset[3] = ECHAR_CAST('Y');	  /* Kanji char set */
    		pstPRI->gl = 0;
    		pstPRI->gr = 2;
    		}
    	else
    ...
    void vt_charshift(const HHEMU hhEmu)
    	...
    	switch(hhEmu->emu_code)
    		{
    	case 0x0F : 			/* LS0 */
    	case ECHAR_CAST('G') :		/* vt52 use ASCII */
    		pstPRI->gl = 0;
    		break;
    	case 0x0E : 			/* LS1 */
    	case ECHAR_CAST('F') :		/* vt52 use graphics */
    		pstPRI->gl = 1;
    		break;
    	case ECHAR_CAST('~') :		/* LS1R */
    		pstPRI->gr = 1;
    		break;
    	case ECHAR_CAST('n') :		/* LS2 */
    		pstPRI->gl = 2;
    		break;
    	case ECHAR_CAST('}') :		/* LS2R */
    		pstPRI->gr = 2;
    		break;
    	case ECHAR_CAST('o') :		/* LS3 */
    		pstPRI->gl = 3;
    		break;
    	case ECHAR_CAST('|') :		/* LS3R */
    		pstPRI->gr = 3;
    		break;
    	case 0x8E : 			/* SS2 */
    	case ECHAR_CAST('N')	:		/* ESC N */
    		pstPRI->old_gl = pstPRI->gl;
    		pstPRI->gl = 2;
    		hhEmu->emu_datain = vt_char_emulatecmd;
    		break;
    	case 0x8F : 			/* SS3 */
    	case ECHAR_CAST('O')	:		/* ESC O */
    		pstPRI->old_gl = pstPRI->gl;
    		pstPRI->gl = 3;
    		hhEmu->emu_datain = vt_char_emulatecmd;
    		break;
  ...
  int vt_char_emulatecmd(const HHEMU hhEmu, const ECHAR ccode)
    ...
    int vt_char_emulatecmd(const HEMU hEmu, const ECHAR ccode)
    	...
    	if (hhEmu->state == 0)
    		{
    		if (IN_RANGE(ccode, ECHAR_CAST(' '), 0x7F) || IN_RANGE(ccode, 0xA0, 0xFF))
    			{
    ...
    void vt_setdtbl(const HHEMU hhEmu, ECHAR tbl[], ECHAR cset)
    	...
    	case ECHAR_CAST('0'):	/* DEC special graphics */
    		...
    		break;
    	case ECHAR_CAST('<'):	/* DEC supplemental graphics */
    		...
    		break;
    	case ECHAR_CAST('B'):	/* normal ASCII */
    	case ECHAR_CAST('Y') :	/* KANJI */
