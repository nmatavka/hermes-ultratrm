...
void vt52PrintCommands(const HHEMU hhEmu)
	...
	switch (code)
		{
	case ECHAR_CAST('^'): 			  /* auto print on */
		hhEmu->print_echo = TRUE;
		break;

	case ECHAR_CAST('_'): 			  /* auto print off */
		hhEmu->print_echo = FALSE;
		break;

	case ECHAR_CAST(']'): 			  /* print screen */
	...
	case ECHAR_CAST('V'): 			  /* print cursor line */
		printEchoLine(hhEmu->hPrintHost,
				hhEmu->emu_apText[row_index(hhEmu, hhEmu->emu_currow)],
				emuRowLen(hhEmu, row_index(hhEmu, hhEmu->emu_currow)));
		break;

	case ECHAR_CAST('W'): 			  /* enter printer controller mode */
		hhEmu->state = 4;		   /* start vt52_prnc() */
		break;

	case ECHAR_CAST('X'): 			  /* exit printer controller mode */
		break;
	default:
		break;
		}
...
void vt52_id(const HHEMU hhEmu)
	...
	sp = L"\033/Z";
	CnvrtHostToECHAR(ech, sizeof(ech), L"\033/Z",
                     StrCharGetByteCount(L"\033/Z"));
