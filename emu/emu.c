...
void char_pn(const HHEMU hhEmu) 	 /* interpret a character as a numeric param */
	{
	if (hhEmu->emu_code < ECHAR_CAST(' '))
		hhEmu->emu_code = ECHAR_CAST(' ');

	hhEmu->selector[hhEmu->selector_cnt] =
	hhEmu->num_param[hhEmu->num_param_cnt] = hhEmu->emu_code - ECHAR_CAST(' ') + 1;
...
void emuStdGraphic(const HHEMU hhEmu)
	...
	if ((hhEmu->stUserSettings.nEmuId == EMU_AUTO) ||
		...
		//		
		if (QueryCLoopHostWidthState(sessQueryCLoopHdl(hhEmu->hSession)))
			{
			if (isHostWideChar(ccode))
				{
				int iColPrev = iCol;
	...
	if (iCol > hhEmu->emu_maxcol)
		...
		if (hhEmu->mode_AWM)
			...
			#if 0
			printEchoChar(hhEmu->hPrintEcho, ECHAR_CAST('\r'));
			printEchoChar(hhEmu->hPrintEcho, ECHAR_CAST('\n'));
			#endif
			printEchoString(hhEmu->hPrintEcho, tp, emuRowLen(hhEmu, iRow));
			CnvrtHostToECHAR(echBuf, sizeof(echBuf), L"\r\n",
...
void emuSendKeyString(const HHEMU hhEmu,
	...
	pbTemp = pstKey->fPointer ? pstKey->u.pachKeyStr : pstKey->u.achKeyStr;
	CLoopBytesOut(sessQueryCLoopHdl(hhEmu->hSession), pbTemp, pstKey->uLen);
	}
...
void emuSendString(const HHEMU hhEmu, ECHAR *str, int nLen)
	{
#if defined(ULTRATERMINAL_UNICODE_ONLY)
	BYTE *pbOut = NULL;
	int nChars = 0;
	int nActualChars = 0;
	int nOut = 0;
	...
	nActualChars = StrCharGetEcharLen(str);
	nChars = ((nLen % (int)sizeof(ECHAR)) == 0) ?
			(nLen / (int)sizeof(ECHAR)) : nLen;
	if (nChars > nActualChars)
		nChars = nActualChars;
	if (nChars <= 0)
		return;

	pbOut = malloc((size_t)nChars * 4 + 4);
	if (pbOut == NULL)
		{
		assert(FALSE);
		return;
	...
	if (DoUTF8)
		{
		nOut = WideCharToMultiByte(CP_UTF8, 0, str, nChars,
				(LPSTR)pbOut, nChars * 4 + 4, NULL, NULL);
		}
	else
#endif
		{
		BOOL fByteClean = TRUE;
		for (i = 0; i < nChars; i++)
			{
			if (str[i] > 0x00FF)
				{
		...
		if (fByteClean)
			{
			for (i = 0; i < nChars; i++)
				pbOut[i] = (BYTE)(str[i] & 0x00FF);
			nOut = nChars;
			}
		else
			{
			nOut = WideCharToMultiByte(CP_ACP, 0, str, nChars,
					(LPSTR)pbOut, nChars * 4 + 4, NULL, NULL);
			}
		}

	if (nOut > 0)
		CLoopBytesOut(sessQueryCLoopHdl(hhEmu->hSession), pbOut, nOut);

	free(pbOut);
#else
        WCHAR *pchHostWide = NULL;
        WCHAR *pchTemp = NULL;
	...
	pchHostWide = malloc((unsigned long)nLen*sizeof(ECHAR));
	if (pchHostWide == NULL)
		{
		assert(FALSE);
		return;
    ...
    if (!DoUTF8) {
#endif
        ulSize = (unsigned long)CnvrtECHARtoHost(pchHostWide, (unsigned long)nLen*sizeof(ECHAR),
											  str, (unsigned long)nLen*sizeof(ECHAR));
#if defined(INCL_VTUTF8)
    } else {
        while (nLen > (int)ulSize) {
            pchHostWide[ulSize++] = (str[ulSize] & 0x00FF);
        }
    }
#endif
	pchTemp = pchHostWide;    
 ...
 *
 */
int _cdecl KeyCmp(const void *pvKey1, const void *pvKey2)
	{
	const KEY *pstKey1 = (const KEY *)pvKey1;
	const KEY *pstKey2 = (const KEY *)pvKey2;
...
int emuCreateTextAttrBufs(const HEMU hEmu, const size_t nRows, size_t nCols)
	...
	//
	if ((hhEmu->emu_apAttr = (PSTATTR *)calloc(nRows, sizeof(LPWSTR))) == 0)
		{
		assert(FALSE);
		emuDestroyTextAttrBufs(hEmu);
		return FALSE;
...
int emuCreateNameTable(const HHEMU hhEmu)
	...
	// EMU_AUTO
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_AUTO, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_ANSI
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_ANSI, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_ANSIW
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_ANSIW, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_MIMI
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_MINI, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_VIEW
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VIEW, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_TTY
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_TTY, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_VT100
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VT100, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_VT52
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VT52, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_VT100J
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VT100J, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_VT220
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VT220, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_VT320
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VT320, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_VT100+
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VT100PLUS, achText, sizeof(achText) / sizeof(WCHAR));
	...
	idx++;
	StrCharCopy(hhEmu->pstNameTable[idx].acName, achText);
	hhEmu->pstNameTable[idx].nEmuId = EMU_VT100PLUS;
#endif

#if defined(INCL_VTUTF8)
	// EMU_VTUTF8
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VTUTF8, achText, sizeof(achText) / sizeof(WCHAR));
	...
	// EMU_VIDEOTEX
	//
	iLen = LoadString(glblQueryDllHinst(), IDS_EMUNAME_VIDEOTEX, achText, sizeof(achText) / sizeof(WCHAR));
	...
	idx++;
	StrCharCopy(hhEmu->pstNameTable[idx].acName, achText);
	hhEmu->pstNameTable[idx].nEmuId = EMU_VIDEOTEX;
#endif
