...
//
#define DEVCLASS		L"comm/datamodem"
...
#define DIRECT_COMSSH	    (DIRECT_COM1+5)
...
#if defined(INCL_SSH)
#define MAX_SSH_HOST_LEN	128
#define MAX_SSH_USER_LEN	128
#define MAX_SSH_PATH_LEN	260
#define MAX_SSH_OPTIONS_LEN	512
#endif
...
struct stCnctDrvPrivate
	...
	WCHAR	 achDest[(TAPIMAXDESTADDRESSSIZE/2)+1],	// local portion of phone num.
			 achAreaCode[10],
	...
	WCHAR	 achLineName[100];	// used in dialing dialog
    WCHAR    achComDeviceName[256]; // used with enumerated ports.
	...
	WCHAR    achSshConfigPath[MAX_SSH_PATH_LEN];
	WCHAR    achSshKnownHostsPath[MAX_SSH_PATH_LEN];
	WCHAR    achSshIdentityFiles[MAX_SSH_OPTIONS_LEN];
	WCHAR    achSshTargetHost[MAX_IP_ADDR_LEN];
	int      iSshTargetPort;
	WCHAR    achSshProxy[MAX_SSH_OPTIONS_LEN];
	WCHAR    achSshForwardings[MAX_SSH_OPTIONS_LEN];
	int      fSshAgent;
#endif
