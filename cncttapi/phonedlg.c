	...
	case WM_COMMAND:
		...
		case IDC_PB_CONFIGURE:
			...
			if ((i = (int)SendDlgItemMessage(hwnd, IDC_CB_MODEMS, CB_GETCURSEL,
					0, 0)) != CB_ERR)
				{
				const LRESULT lrItemData = SendDlgItemMessage(hwnd,
						IDC_CB_MODEMS, CB_GETITEMDATA, (WPARAM)i, 0);

				pstLineIds = (PSTLINEIDS)lrItemData;

				if (lrItemData != CB_ERR)
					{
					if ((INT_PTR)pstLineIds != CB_ERR)
						{
						...
						if (IN_RANGE(pstLineIds->dwPermanentLineId,
                            ...
                            cncttapiGetCOMSettings(pstLineIds->dwLineId,
                                                   achSettings,
                                                   sizeof(achSettings) / sizeof(WCHAR));
...
STATIC_FUNC int tapi_SAVE_NEWPHONENUM(HWND hwnd)
    ...
    if (IsWindowEnabled(GetDlgItem(hwnd, IDC_EB_HOSTADDR)))
        ...
        GetDlgItemText(hwnd, IDC_EB_HOSTADDR, hhDriver->achDestAddr,
			sizeof(hhDriver->achDestAddr)/sizeof(WCHAR));
        }
...
void ResetComboBox(const HWND hwnd)
	...
	if ((lr = SendMessage(hwnd, CB_GETCOUNT, 0, 0)) != CB_ERR)
		...
		for (i = 0 ; i < lr ; ++i)
			{
			const LRESULT lrItemData =
				SendMessage(hwnd, CB_GETITEMDATA, (WPARAM)i, 0);
...
static int VerifyAddress(const HWND hwnd)
	...
	WCHAR achAreaCode[10];
	WCHAR achDest[(TAPIMAXDESTADDRESSSIZE/2)+1];
	WCHAR ach[256];
	...
	MSG_EXIT:
		...
		hhDriver->achDisplayableDest[0] = L'\0';
...
int cncttapiGetCOMSettings( const DWORD dwLineId, LPWSTR pachStr, const size_t cb )
    {
    ...
    lReturn = cncttapiGetLineConfig(dwLineId, (VOID **)&pvs);
    if (lReturn != 0)
        return lReturn;
