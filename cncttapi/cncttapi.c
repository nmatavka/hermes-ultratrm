...
static int tapiValidateDevConfig(const LPVARSTRING pvs);
static PUMDEVCFG tapiDevConfigPtr(const LPVARSTRING pvs);

const WCHAR *g_achApp = L"UltraTerminal";
...
static void cnctdrvSshSetting(HCOM hCom, const WCHAR *pszName,
    ...
    {
    WCHAR achInstruction[1024];
    WCHAR achResult[64];

    achInstruction[0] = L'\0';
    lstrcpyn(achInstruction, L"SET ",
            sizeof(achInstruction) / sizeof(WCHAR));
    lstrcat(achInstruction, pszName);
    lstrcat(achInstruction, L"=");
    if (pszValue != 0)
        lstrcat(achInstruction, pszValue);

    ComDriverSpecial(hCom, achInstruction, achResult,
            sizeof(achResult) / sizeof(WCHAR));
    }

static void cnctdrvSshSettingInt(HCOM hCom, const WCHAR *pszName, int nValue)
    {
    WCHAR achValue[32];

    wsprintf(achValue, L"%d", nValue);
    cnctdrvSshSetting(hCom, pszName, achValue);
    }

static int cnctdrvConfigureSsh(const HHDRIVER hhDriver, HCOM hCom)
    {
    int nMode;
    int iPort;
    int iTargetPort;
    ...
    iTargetPort = hhDriver->iSshTargetPort > 0 ?
            hhDriver->iSshTargetPort : hhDriver->iPort;
    if (iTargetPort <= 0)
        iTargetPort = sessQueryTelnetPort(hhDriver->hSession);
    if (iTargetPort <= 0)
        iTargetPort = 23;

    pszSshHost = hhDriver->achSshHost[0] != L'\0' ?
            hhDriver->achSshHost : hhDriver->achDestAddr;
    ...
    cnctdrvSshSettingInt(hCom, L"AGENT", hhDriver->fSshAgent);
...
static int tapiValidateDevConfig(const LPVARSTRING pvs)
    ...
    if (pvs->dwTotalSize < sizeof(VARSTRING))
        return FALSE;

    if (pvs->dwNeededSize > pvs->dwTotalSize)
        return FALSE;

    if (pvs->dwStringOffset < sizeof(VARSTRING))
        return FALSE;

    if (pvs->dwStringOffset > pvs->dwTotalSize)
        return FALSE;

    cbTail = pvs->dwTotalSize - pvs->dwStringOffset;
    if (pvs->dwStringSize < sizeof(UMDEVCFG) ||
            pvs->dwStringSize > cbTail)
        return FALSE;
...
static PUMDEVCFG tapiDevConfigPtr(const LPVARSTRING pvs)
    {
    if (!tapiValidateDevConfig(pvs))
        return NULL;

    return (PUMDEVCFG)((BYTE *)pvs + pvs->dwStringOffset);
    }
...
int WINAPI cnctdrvInit(const HHDRIVER hhDriver)
	...
	if ((lRet = lineInitialize(&hhDriver->hLineApp, glblQueryDllHinst(),
		...
		if ( id )
			{
			LoadString(glblQueryDllHinst(), id, ach, sizeof(ach) / sizeof(WCHAR));
	...
	hhDriver->achDest[0]		= L'\0';
	hhDriver->achAreaCode[0]	= L'\0';
	hhDriver->achLineName[0]	= L'\0';
	...
	hhDriver->achSshHost[0] = L'\0';
	hhDriver->iSshPort = 22;
	hhDriver->achSshUser[0] = L'\0';
	hhDriver->achSshConfigPath[0] = L'\0';
	hhDriver->achSshKnownHostsPath[0] = L'\0';
	hhDriver->achSshIdentityFiles[0] = L'\0';
	hhDriver->achSshTargetHost[0] = L'\0';
	hhDriver->iSshTargetPort = 0;
	hhDriver->achSshProxy[0] = L'\0';
	hhDriver->achSshForwardings[0] = L'\0';
	hhDriver->fSshAgent = TRUE;
#endif
...
int WINAPI cnctdrvLoad(const HHDRIVER hhDriver)
	...
	ul = sizeof(hhDriver->nTransport);
	sfGetSessionItem(sfhdl, SFID_CNCT_TRANSPORT, &ul, &hhDriver->nTransport);
	if (hhDriver->nTransport < CNCT_TRANSPORT_DIRECT_TCP ||
			hhDriver->nTransport > CNCT_TRANSPORT_SSH_TUNNEL)
		hhDriver->nTransport = CNCT_TRANSPORT_DIRECT_TCP;

	hhDriver->achSshHost[0] = L'\0';
	ul = sizeof(hhDriver->achSshHost);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_HOST, &ul, hhDriver->achSshHost);

	hhDriver->iSshPort = 22;
	ul = sizeof(hhDriver->iSshPort);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_PORT, &ul, &hhDriver->iSshPort);

	hhDriver->achSshUser[0] = L'\0';
	ul = sizeof(hhDriver->achSshUser);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_USER, &ul, hhDriver->achSshUser);

	hhDriver->achSshConfigPath[0] = L'\0';
	ul = sizeof(hhDriver->achSshConfigPath);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_CONFIG, &ul,
		hhDriver->achSshConfigPath);

	hhDriver->achSshKnownHostsPath[0] = L'\0';
	ul = sizeof(hhDriver->achSshKnownHostsPath);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_KNOWN_HOSTS, &ul,
		hhDriver->achSshKnownHostsPath);

	hhDriver->achSshIdentityFiles[0] = L'\0';
	ul = sizeof(hhDriver->achSshIdentityFiles);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_IDENTITIES, &ul,
		hhDriver->achSshIdentityFiles);

	hhDriver->achSshTargetHost[0] = L'\0';
	ul = sizeof(hhDriver->achSshTargetHost);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_TARGET_HOST, &ul,
		hhDriver->achSshTargetHost);

	hhDriver->iSshTargetPort = 0;
	ul = sizeof(hhDriver->iSshTargetPort);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_TARGET_PORT, &ul,
		&hhDriver->iSshTargetPort);

	hhDriver->achSshProxy[0] = L'\0';
	ul = sizeof(hhDriver->achSshProxy);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_PROXY, &ul, hhDriver->achSshProxy);

	hhDriver->achSshForwardings[0] = L'\0';
	ul = sizeof(hhDriver->achSshForwardings);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_FORWARDINGS, &ul,
		hhDriver->achSshForwardings);

	hhDriver->fSshAgent = TRUE;
	ul = sizeof(hhDriver->fSshAgent);
	sfGetSessionItem(sfhdl, SFID_CNCT_SSH_AGENT, &ul, &hhDriver->fSshAgent);
#endif
...
int WINAPI cnctdrvSave(const HHDRIVER hhDriver)
	...
	sfPutSessionItem(sfhdl, SFID_CNCT_AREA,
		(lstrlen(hhDriver->achAreaCode) + 1) * sizeof(WCHAR),
			hhDriver->achAreaCode);

	sfPutSessionItem(sfhdl, SFID_CNCT_DEST,
		(lstrlen(hhDriver->achDest) + 1) * sizeof(WCHAR), hhDriver->achDest);
...
#if defined (INCL_SSH)
	sfPutSessionItem(sfhdl, SFID_CNCT_TRANSPORT,
		sizeof(hhDriver->nTransport), &hhDriver->nTransport);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_HOST,
		(lstrlen(hhDriver->achSshHost) + 1) * sizeof(WCHAR),
		hhDriver->achSshHost);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_PORT,
		sizeof(hhDriver->iSshPort), &hhDriver->iSshPort);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_USER,
		(lstrlen(hhDriver->achSshUser) + 1) * sizeof(WCHAR),
		hhDriver->achSshUser);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_CONFIG,
		(lstrlen(hhDriver->achSshConfigPath) + 1) * sizeof(WCHAR),
		hhDriver->achSshConfigPath);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_KNOWN_HOSTS,
		(lstrlen(hhDriver->achSshKnownHostsPath) + 1) * sizeof(WCHAR),
		hhDriver->achSshKnownHostsPath);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_IDENTITIES,
		(lstrlen(hhDriver->achSshIdentityFiles) + 1) * sizeof(WCHAR),
		hhDriver->achSshIdentityFiles);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_TARGET_HOST,
		(lstrlen(hhDriver->achSshTargetHost) + 1) * sizeof(WCHAR),
		hhDriver->achSshTargetHost);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_TARGET_PORT,
		sizeof(hhDriver->iSshTargetPort), &hhDriver->iSshTargetPort);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_PROXY,
		(lstrlen(hhDriver->achSshProxy) + 1) * sizeof(WCHAR),
		hhDriver->achSshProxy);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_FORWARDINGS,
		(lstrlen(hhDriver->achSshForwardings) + 1) * sizeof(WCHAR),
		hhDriver->achSshForwardings);
	sfPutSessionItem(sfhdl, SFID_CNCT_SSH_AGENT,
		sizeof(hhDriver->fSshAgent), &hhDriver->fSshAgent);
#endif
...
int DoAnswerCall(const HHDRIVER hhDriver)
    ...
    if (TRAP(lineOpen(hhDriver->hLineApp, hhDriver->dwLine, &hhDriver->hLine,
        ...
        {
        assert(0);
	    LoadString(glblQueryDllHinst(), IDS_ER_CNCT_TAPIFAILED, ach, sizeof(ach) / sizeof(WCHAR));
    ...
    // mrw:9/18/96
    //
	LoadString(glblQueryDllHinst(), IDS_GNRL_APPNAME, ach, sizeof(ach) / sizeof(WCHAR));
...
ERROR_EXIT:
        ...
        if ( msgFlag )
            {
            LoadString(glblQueryDllHinst(), uidErr, ach, sizeof(ach) / sizeof(WCHAR));
...
int WINAPI cnctdrvConnect(const HHDRIVER hhDriver, const unsigned int uFlags)
	...
	//
	WCHAR	szInstruct[MAX_IP_ADDR_LEN+11+1]; // Used only for WinSock
	WCHAR   szResult[MAX_IP_ADDR_LEN+11+1];   // Used only for WinSock
	int     iNumChars;
#endif
	WCHAR	achNewCnct[FNAME_LEN];
	WCHAR	achCom[20];
	...
	LoadString(glblQueryDllHinst(),	IDS_GNRL_NEW_CNCT, achNewCnct,
		sizeof(achNewCnct) / sizeof(WCHAR));

	if (ach[0] == L'\0' || lstrcmp(achNewCnct, ach) == 0)
		{
		...
		hhDriver->achDestAddr[MAX_IP_ADDR_LEN - 1] = L'\0';
		hhDriver->dwPermanentLineId = DIRECT_COMWINSOCK;
		}
#endif
	...
	if (!IN_RANGE(hhDriver->dwPermanentLineId, DIRECT_COM1, DIRECT_COM4) &&
        ...
        if (hhDriver->dwPermanentLineId != DIRECT_COMWINSOCK &&
                hhDriver->dwPermanentLineId != DIRECT_COMSSH &&
                (hhDriver->achDest[0] == L'\0' || 
				 hhDriver->achDialableDest[0] == L'\0' ||
				 hhDriver->achCanonicalDest[0] == L'\0'))
            {
            fFlag = TRUE;
            }
		}
...
NEWPHONEDLG:
...
#if defined(INCL_SSH)
	if (hhDriver->dwPermanentLineId == DIRECT_COMSSH ||
			(hhDriver->dwPermanentLineId == DIRECT_COMWINSOCK &&
			 hhDriver->nTransport != CNCT_TRANSPORT_DIRECT_TCP))
		{
		int iPort;
		...
		cnctdrvConfigureSsh(hhDriver, hCom);
	...
	if (hhDriver->dwPermanentLineId == DIRECT_COMWINSOCK)
		...
		// size of MAX_AP_ADDR_LEN.  REV 09/20/2000 
		//
		StrCharCopy(szInstruct, L"SET IPADDR=");
		iNumChars = StrCharGetStrLength(szInstruct);
		StrCharCopyN(&szInstruct[iNumChars], hhDriver->achDestAddr,
			         sizeof(szInstruct)/sizeof(WCHAR) - iNumChars);

		//
		// Make sure the string is null terminated.
		//
		szInstruct[sizeof(szInstruct)/sizeof(WCHAR) - 1]=L'\0';
    ...
    if (hXfer != NULL)
        ...
        if (pX != NULL &&
            ...
            if (uFlags & CNCT_XFERABORTCONFIRM)
                ...
                //
                LoadString(glblQueryDllHinst(), IDS_ER_CNCT_ACTIVETRANSFER, ach, sizeof(ach) / sizeof(WCHAR));
...
int Handoff(const HHDRIVER hhDriver)
	...
	if (pVarstr->dwStringOffset < sizeof(VARSTRING) ||
			pVarstr->dwStringOffset > pVarstr->dwTotalSize ||
			pVarstr->dwStringSize < sizeof(HANDLE) ||
			pVarstr->dwStringSize >
					pVarstr->dwTotalSize - pVarstr->dwStringOffset)
		{
		assert(FALSE);
		free(pVarstr);
        pVarstr = NULL;
...
static int tapiReinitMessage(const HHDRIVER hhDriver)
	...
	LoadString(glblQueryDllHinst(), IDS_ER_TAPI_REINIT, ach, sizeof(ach) / sizeof(WCHAR));

	LoadString(glblQueryDllHinst(), IDS_ER_TAPI_REINIT2, achTitle,
		sizeof(achTitle) / sizeof(WCHAR));
...
static int DoNewModemWizard(HWND hWnd, int iTimeout)
    ...
    if(mscAskWizardQuestionAgain())
        {
		LoadString(glblQueryDllHinst(), IDS_ER_CNCT_BADLINE, ach, sizeof(ach) / sizeof(WCHAR));

		if (TimedMessageBox(hWnd, ach, 0, MB_YESNO | MB_ICONEXCLAMATION, iTimeout) == IDYES)
			{
            WCHAR  systemDir[MAX_PATH];
            WCHAR  executeString[MAX_PATH * 3];
            WCHAR *pParams = L"\\control.exe\" modem.cpl,,Add";
            UINT   numChars = 0;

            WCHAR_Fill(systemDir, L'\0', MAX_PATH);
            WCHAR_Fill(executeString, L'\0', MAX_PATH * 3);
            ...
            if (numChars == 0 || StrCharGetStrLength(systemDir) == 0)
                ...
                {
                if (StrCharGetStrLength(systemDir) + StrCharGetStrLength(pParams) + sizeof(L"\"") / sizeof(WCHAR) >
                    sizeof(executeString) / sizeof(WCHAR))
                    {
                    returnVal = -2;
                    }
                else
                    {
                    StrCharCopy(executeString, L"\"");
        ...
        if (IsNT())
            ...
            if (hCnct)
                ...
                if (iStatus != CNCT_STATUS_FALSE &&
                    ...
                    //
                    LoadString(glblQueryDllHinst(), IDS_ER_TAPI_NEEDS_RESET, ach, sizeof(ach) / sizeof(WCHAR));
